...
describe("SyncUserFromWebhookUseCase", () => {
  let mockPrisma: any;
  const ctx = (actor: any = { type: "system", reason: "test" }) =>
    createAppContext({ actor, prisma: mockPrisma });
  ...
  it("creates a new local user and preserves isPatron: false", async () => {
    ...
    const createSpy = mockPrisma.user.create.mockResolvedValue({} as any);

    await SyncUserFromWebhookUseCase.execute(
      ctx(),
      {
        id: "u1",
  ...
  it("updates existing local user and preserves current isPatron state", async () => {
    ...
    const updateSpy = mockPrisma.user.update.mockResolvedValue({} as any);
    ...
    expect(updateSpy.mock.calls[0][0].data).not.toHaveProperty("isPatron");
    // Note: identity sync does not pass isPatron to the update payload.
  });
  ...
  it("performs soft delete correctly with full anonymization", async () => {
    ...
    expect(mockPrisma.patronGrant.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(EmailService.sendAccountDeletedEmail).toHaveBeenCalledWith(
      "john@example.com",
    );
  });
