...
function makeDb() {
  const tx = {
    user: { findUnique: vi.fn().mockResolvedValue({ id: 'user_opaque_1', email: 'person@example.com' }) },
    ...
    emailPreference: {
      upsert: vi.fn().mockResolvedValue({ id: 'pref_1' }),
      findUnique: vi.fn().mockResolvedValue({ marketingEmails: true }),
    },
...
describe('signed content unsubscribe flow', () => {
  ...
  it('valid POST unsubscribes and repeat POST is idempotent while leaving systemEmails and PatronGrant untouched', async () => {
    const { db, tx } = makeDb();
    ...
    expect(tx.emailPreference.upsert).toHaveBeenCalledWith(expect.objectContaining({
      where: { email: 'person@example.com' },
      update: expect.objectContaining({ marketingEmails: false, unsubscribedAt: expect.any(Date) }),
      create: expect.objectContaining({ marketingEmails: false, systemEmails: true }),
    }));
  ...
  it('registration, tipping and PatronGrant do not create content consent in the signed unsubscribe flow', async () => {
    ...
    expect(tx.emailPreference.upsert).toHaveBeenCalledTimes(1);
    expect(tx.emailPreference.upsert).not.toHaveBeenCalledWith(expect.objectContaining({ update: expect.objectContaining({ marketingEmails: true }) }));
