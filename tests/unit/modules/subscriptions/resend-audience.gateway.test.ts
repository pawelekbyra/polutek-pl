...
describe('ResendAudienceGateway explicit consent sync', () => {
  ...
  it('creates subscribed contact after explicit subscribe when no existing contact is returned', async () => {
  ...
  it('returns FAILED when subscribe provider sync fails', async () => {
    resend.contacts.get.mockRejectedValue(new Error('down'));
    resend.contacts.create.mockRejectedValue(new Error('down'));
    resend.contacts.update.mockRejectedValue(new Error('down'));
    const gateway = new ResendAudienceGateway(resend as any);
