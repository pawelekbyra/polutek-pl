...
describe('PremiumWrapper playback plan state messaging', () => {
  ...
  it('uses keyboard-accessible actions across lock and informational overlay paths', () => {
    ...
    expect(accessLockOverlaySource).toContain('aria-label={isPl ? "Zaloguj się" : "Sign in"}');
  ...
  it('keeps compact playlist locks distinct from the full hero overlay', () => {
    ...
    expect(accessLockOverlaySource).toContain('compactLabel: isPl ? "PATRON" : "PATRON"');
    expect(accessLockOverlaySource).toContain('compactLabel: isPl ? "LOGIN" : "SIGN IN"');
  });
});
