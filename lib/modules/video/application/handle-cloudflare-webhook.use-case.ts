...
export async function handleCloudflareStreamWebhook(
  ...
  const updatedAsset = await (ctx.prisma as any).$transaction(async (tx: any) => {
    ...
    if (newState === VIDEO_ASSET_PROCESSING_STATE.READY) {
        ...
        // Auto-promuj do primary tylko jeśli nie istnieje żaden inny READY primary asset.
        // Jeśli admin ma już aktywne źródło, nie przełączamy automatycznie — decyzja należy do admina.
        const existingReadyPrimary = await tx.videoAsset.findFirst({
          where: { videoId: asset.videoId, id: { not: asset.id }, isPrimary: true, processingState: VIDEO_ASSET_PROCESSING_STATE.READY },
        });
