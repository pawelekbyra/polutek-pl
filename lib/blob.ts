...
import {
  MediaPolicy,
  MediaHostEnv,
  parseMediaHosts as _parseMediaHosts,
...
export function isAllowedCommentImageUrl(
  rawUrl: string,
  env: MediaHostEnv = process.env,
) {
...
function isConfiguredVercelBlobUrl(rawUrl: string) {
  try {
    ...
    const blobHosts = new Set(
      _parseMediaHosts(process.env.NEXT_PUBLIC_BLOB_PUBLIC_HOST),
    );
...
export async function getGatedBlobResponse(
  ...
  try {
    ...
    accessResult = await checkVideoAccess({ videoIdOrSlug: videoId }, ctx);
  } catch (error) {
    logger.warn(
      "[getGatedBlobResponse] Actor resolution failed",
  ...
  if (!decision.hasAccess) {
    recordMetric(
      "media_proxy.access_denied",
      {
        videoId,
        reason: decision.reason || "unknown",
        requiredTier: decision.requiredTier || "unknown",
      },
      { level: "warn" },
    );
    return new NextResponse("Forbidden", { status: 403 });
  ...
  try {
    ...
    logger.info(
      `[MediaProxy] Fetching configured media host: ${targetHost} (Range: ${range?.value || "none"})`,
    );
    ...
    if (!response.ok && response.status !== 206) {
      logger.error(
        `[MediaProxy] Upstream media error: ${response.status} ${response.statusText} from ${targetHost}`,
      );
      recordAlert("media_proxy.upstream_error", {
        videoId,
        status: response.status,
        host: targetHost,
      });
      return new NextResponse("Error fetching content", {
        status: response.status,
      });
    }
