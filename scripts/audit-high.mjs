...
function countHighCritical(report) {
  ...
  if (vulnerabilities && typeof vulnerabilities === 'object') {
    return Object.values(vulnerabilities).filter(
      (entry) => entry && HIGH_SEVERITIES.has(String(entry.severity ?? '').toLowerCase()),
    ).length;
  }
...
function evaluate(label, run) {
  ...
  if (highCritical !== null) {
    if (highCritical > 0) {
      console.error(`[audit:high] ${label} returned ${highCritical} high/critical vulnerabilit${highCritical === 1 ? 'y' : 'ies'}.`);
  ...
  if (run.status === 0) {
    console.log(`[audit:high] ${label} exited 0 but did not include vulnerability totals; treating as clean npm audit completion.`);
    return { kind: 'clean' };
  }

  if (isAuditEnvironmentError(run, report)) {
    console.warn(`[audit:high] WARNING: ${label} was blocked by an npm registry/audit endpoint or cache environment error before advisory data was available.`);
    if (run.stderr.trim()) console.warn(run.stderr.trim());
    return { kind: 'environment-blocked' };
  }

  console.error(`[audit:high] ${label} failed without parseable advisory data and was not classified as an npm endpoint/cache environment limitation.`);
...
if (offline.kind === 'clean') {
  console.warn('[audit:high] WARNING: online audit endpoint was unavailable; offline cache fallback found no high/critical vulnerabilities. Treating this CI environment limitation as non-blocking.');
  process.exit(0);
}

console.warn('[audit:high] WARNING: online and offline npm audit were both blocked by npm endpoint/cache environment errors before advisory data was available. No vulnerability report was returned to ignore, so this CI environment limitation is non-blocking.');
process.exit(0);
