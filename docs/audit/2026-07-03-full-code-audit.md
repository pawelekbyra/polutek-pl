...
- **W6** — `PATRON_MIN_TIP_AMOUNT` / `PATRON_MIN_TIP_CURRENCY` są podpięte do progów płatności i wpływają na eligibility patrona.
- **Ś1** — `warning_closed` nie zawiesza patrona; zamknięte inquiry bez chargebacku zostawia dostęp bez zmian.
