...
| `app/api/videos/[id]/playback-event/route.ts` | Video/Analytics | 1 | R6/R3 | Persistence of events/views |
...
| `app/api/admin/subscribers/resync/route.ts` | Users | 1 | R5 | Legacy subscriber sync |
| `app/api/media/[...path]/route.ts` | Media | 1 | R3 | Legacy media delivery check |

## Analysis (Reconciled After R9 & Subscriptions)
...
- **R5 Users/Referrals/Resync**: Legacy logic persists in admin user details, subscriber resync, and referral claims. `app/api/user/referrals/route.ts` is now clean.
