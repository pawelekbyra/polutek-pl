...
This report records the important findings from the deep architecture/codebase audit performed after the video provider lifecycle, publication/hero/sidebar, and playback/access refactors. The current executable ticket at the time of this report was `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001`.

That ticket was already in progress and this report did not change its scope.

## Executive decision

Do not start a broad rewrite. The codebase is not structurally broken. Add a narrow follow-up ticket after active payments work and before the next non-payments domain ticket:
...
| Payments fulfillment trusts Stripe intent metadata user id without local payment-user cross-check. | P1 security/data correctness | `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001` | Owned by active payments ticket and addressed in PR #996 pending review/merge. |
| Payment request idempotency relies on weak local metadata lookup. | P1 financial correctness | `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001` | Owned by active payments ticket and addressed in PR #996 pending review/merge. |
| Dead legacy Stripe fulfillment/webhook service paths remain in code. | P2 footgun | `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001` | PR #996 delegates legacy fulfillment to the hardened modular use case so no second metadata-trusting production fulfillment remains. |
...
Because payments work was already the current executable ticket, do not insert another ticket before it. After payments hardening completes, the recommended order is:
