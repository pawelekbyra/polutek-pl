...
- Ensure CI has a job that runs the real Vitest suite or clearly distinguish any narrow smoke/critical-path command from full test coverage.
...
- payment metadata-user and request-id idempotency findings belong to the active payments ticket and must not be duplicated here;
- dead legacy payment service paths should be deleted by the payments ticket if they remain unused;
...
- Do not edit `docs/tickets/ready/PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001.md` while that ticket is in progress.
...
- Active payments ticket remains untouched.
