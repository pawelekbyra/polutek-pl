...
UNRESOLVED / SECURITY_REMEDIATION_REQUIRED. Evidence is documented in `docs/reports/reconciliation/POST-929-EMERGENCY-CONTROL-PLANE-RECONCILIATION.md`; production/provider state is not certified by this card.
...
## Expected outcome
Remediate or explicitly plan high npm audit findings after CI exposes security status without suppressing vulnerabilities.

## Allowed scope
Dependency/security changes only after a dedicated ticket expansion defines exact package files and upgrade strategy. A future activated ticket must list exact allowed paths before work starts.
