...
| 3.4 | `INCOMPLETE-006` — Stripe reconciliation job | `DONE` | PR #1337 — Implemented Vercel cron reconciliation job in `app/api/cron/stripe-reconciliation`. |
...
| 4.3 | `CLEANUP-001` — legacy service layer migration/removal | `DONE` | PR #1337 — Migrated `email.service.ts` and `profile.service.ts`. Legacy service layer removed. |
...
- [x] `INCOMPLETE-006` — Stripe reconciliation job — `DONE` in PR #1337.
...
- [x] `CLEANUP-001` — legacy service layer migration/removal map execution — `DONE` in PR #1337. All legacy services removed.
...
- `#1337` — INCOMPLETE-006: Stripe reconciliation cron; CLEANUP-001: legacy services removal.
