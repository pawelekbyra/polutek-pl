# ADMIN-VIDEO-PUBLICATION-AND-HERO-CONTRACT-001 — Publication, Hero and video state contract

Status: NON_EXECUTABLE / PLANNED
Ticket ID: ADMIN-VIDEO-PUBLICATION-AND-HERO-CONTRACT-001
...
## Purpose
Publication, Hero and video state contract.

## Associated risk IDs
VIDEO-PUBLISH-001, VIDEO-HERO-001, VIDEO-STATE-001

## Current evidence classification
CONFIRMED_GAP / CONTRACT_REPAIR_REQUIRED. Evidence is documented in `docs/reports/reconciliation/POST-929-EMERGENCY-CONTROL-PLANE-RECONCILIATION.md`; production/provider state is not certified by this card.

## Dependencies and ordering
Depends on: TUS upload lifecycle ticket or explicit reconciled decision to split earlier. The canonical order remains in `docs/tickets/ready/README.md` and `docs/roadmap/Launch-Execution-Backlog.md`.

## Expected outcome
Define and enforce publication diagnostics, stable publishedAt semantics, Hero invariants and archive/unpublish transitions.

## Allowed scope
Focused video publication/action policy, UI and tests enumerated before activation. A future activated ticket must list exact allowed paths before work starts.
