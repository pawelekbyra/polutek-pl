# ADMIN-VIDEO-TUS-UPLOAD-LIFECYCLE-001 — Real TUS upload and upload-attempt lifecycle

Status: NON_EXECUTABLE / PLANNED
Ticket ID: ADMIN-VIDEO-TUS-UPLOAD-LIFECYCLE-001
...
## Purpose
Real TUS upload and upload-attempt lifecycle.

## Associated risk IDs
VIDEO-CF-001, VIDEO-CF-002, VIDEO-CF-003, VIDEO-CF-008, VIDEO-CF-009, VIDEO-CF-011, VIDEO-CF-012

## Current evidence classification
PARTIAL_IMPLEMENTATION_MERGED_IN_PR_926 / CORRECTIVE_WORK_REQUIRED. Evidence is documented in `docs/reports/reconciliation/POST-929-EMERGENCY-CONTROL-PLANE-RECONCILIATION.md`; production/provider state is not certified by this card.

## Dependencies and ordering
Depends on: Cloudflare containment and provider privacy evidence path. The canonical order remains in `docs/tickets/ready/README.md` and `docs/roadmap/Launch-Execution-Backlog.md`.

## Expected outcome
Implement/prove correct TUS contract, resume behavior, duplicate attempt handling, asset lifecycle and webhook reconciliation.

## Allowed scope
Focused upload lifecycle files only; schema/package changes require explicit pre-activation reconciliation. A future activated ticket must list exact allowed paths before work starts.
