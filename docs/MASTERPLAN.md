...
| `PAYMENTS-TRUTH-001` | Payment fulfillment must validate against local Payment truth, not mutable provider metadata | `REPOSITORY_EVIDENCE` | `CURRENT_TICKET` | **P0/P1 FINANCIAL + ACCESS CORRECTNESS** | `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001` |
| `PAYMENTS-IDEMPOTENCY-001` | Checkout request idempotency needs local, race-resistant backing | `REPOSITORY_EVIDENCE` | `CURRENT_TICKET` | **P1 FINANCIAL CORRECTNESS** | `PAYMENTS-FULFILLMENT-IDEMPOTENCY-HARDENING-001` |
...
- Do not interrupt this ticket with CI-signal work while it is already in progress.
...
2. `CI-SIGNAL-RECONCILIATION-002` — post-payments CI/test/control-plane signal reconciliation from the architecture audit.
