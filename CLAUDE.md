...
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx`), replacing Vidstack's stock `DefaultVideoLayout`. On 2026-07-14 the owner re-skinned them to a **minimalist Netflix-inspired** style (inspired by the Notflix theme from player.style): the control bar uses a black gradient scrim at bottom with no glass effect; white icons on transparent buttons (`opacity:.8` hover, `opacity:.6` active); white progress bar (no color gradient), simple small thumb; white large rounded play button in center (2px border, white text on white bg); settings menu is dark (`rgba(0,0,0,.95)`) with rounded corners and minimal borders. No glass blur, no fractals, no decorative effects—pure minimalism with smooth opacity/scale transitions. They use Lucide icons, a deliberate replay state, 36 px desktop / 40 px mobile targets, and a 4 px desktop / 5 px mobile progress track inside a 24 px hit area. Visible ±10-second seek buttons and Picture-in-Picture are intentionally absent. Do not restore the liquid glass or doodle experiment unless the owner asks. `PlayerLoadingState.tsx`, `AccessLockOverlay.tsx`, `PlayerErrorOverlay.tsx` and `PlayerStateFrame.tsx` own matching application-style player states; do not restore either the former black loading bar or the doodle player experiment. Player control CSS stays embedded via `<style jsx global>` inside `PolutekControls.tsx`; loading CSS stays beside `PlayerLoadingIndicator`. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]` rules are leftover from the old default skin.
