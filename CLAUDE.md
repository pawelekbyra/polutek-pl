...
1. `app/components/Navbar.tsx`: two module-level `const TEMP_HIDE_LANGUAGE_SWITCHER = true` / `const TEMP_HIDE_NOTIFICATIONS_BELL = true` flags. The language-switcher pill's JSX is wrapped in `{!TEMP_HIDE_LANGUAGE_SWITCHER && (...)}`; the signed-in `<NotificationsMenu />` (the "dzwoneczek"/bell) render is wrapped in `TEMP_HIDE_NOTIFICATIONS_BELL ? null : (...)` inside the existing `isSignedIn` branch (the `isLoaded`-signed-out and skeleton branches are untouched). **To revert:** flip both constants to `false`, or delete the flags and the two wrapping conditionals so the JSX renders unconditionally again exactly as before.
...
3. `app/components/BrandName.tsx` (added 2026-09-20, second pass same day): a module-level `const TEMP_LOGO_AS_TEXT = true` flag. When true, the component returns a text wordmark (`KUTASHI` in `var(--chan-ink)` + `.COM` in `var(--chan-blue)`, the site's primary accent) instead of the permanent glasses-mark `<img src="/logo-glasses.svg">`; the original `<img>` JSX is left completely intact directly below the temporary branch, unreached while the flag is `true`. This is the Navbar's only current mount point (`Navbar.tsx` renders `<BrandName decorative className="h-[28px] md:h-[34px]" />` inside the home link). **To revert:** flip the constant to `false` (or delete the flag and its `if` branch) so the function falls through to the untouched `<img>` return. `tests/unit/components/brand-name.test.tsx` was rewritten to assert the temporary text-wordmark behavior; its own header comment says to restore the original `<img>`-based assertions (accessible image, `alt="www.pawelperfect.pl"`, `src="/logo-glasses.svg"`) when this flag reverts.
4. `app/components/legal/LegalDocs.tsx` (added 2026-09-20, third pass same day): a module-level `const TEMP_HIDE_TERMS_EN_SUMMARY = true` flag. It wraps only the `<LegalSummary>` block at the top of `TermsContentEn()` (the "W skrócie" bullet list — "Watching public videos is free...", "A one-time payment...", "You get access right away...", "Complaints: email ...") in `{!TEMP_HIDE_TERMS_EN_SUMMARY && (...)}`. Scoped to the English Terms of Service only — the Polish Regulamin's own `LegalSummary` call (with different, Polish bullet text) is untouched, as are Polityka Prywatności's and Privacy Policy's. **To revert:** flip the constant to `false`, or delete the flag and the wrapping conditional so the block renders unconditionally again.
5. These flags are the *only* changes in this registry; do not extend "temporary test mode" to any other component without a fresh explicit request, and do not let this pattern (a `TEMP_*` boolean short-circuiting a component or function) become a general convention — it exists here only because the owner explicitly asked for an easy, obviously-temporary, single-file revert per change.

Note for future agents: a second, still-open PR (#1488) is in flight adding a third temporary flag (`TEMP_LOGO_AS_TEXT` in `BrandName.tsx`) and restructuring this section into a "TEMPORARY CHANGES REGISTRY". Once that merges, this section will reflect that structure; until then, this bullet list is the current authoritative set of temporary flags.
