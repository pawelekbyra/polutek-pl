...
- The legacy paper/ink hand-drawn implementation (rough.js `Frame`/`HachureFill`/`NajsSeparator`, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13; `roughjs`/`rough-notation` remain intentionally absent. On 2026-07-14 the owner directed a single cohesive **premium/editorial** style across the whole public site: the homepage, the channel page (`.channel-page-shell`), the custom player controls, and every player state overlay now share one language — soft token borders, an inset top highlight, layered ambient shadows (no hard `Npx Npx 0` offset "3D" shadows), uniform radii, pill controls, and editorial eyebrows. The earlier light comic character on the public/channel shells (wonky multi-radius corners, rotated avatar, hard blue offset shadows) was flattened as part of this pass, and the legacy always-on 1px button `outline` inside `.public-visual-shell`/`.channel-page-shell` was removed (it double-framed already-styled buttons like search, likes and comment reactions). The **admin** shell is out of scope and keeps its own approved treatment. Lean on the `--chan-*` tokens (and `color-mix` for tinted shadows) so `/testujemy` and `/cyberpunk-light` keep re-theming; avoid new literal hexes and hard offset shadows. Do not restore the old rough.js system or global paper utilities. `NajsIcon` remains unrelated shared icon infrastructure.
