...
- The legacy paper/ink hand-drawn implementation (rough.js `Frame`/`HachureFill`/`NajsSeparator`, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13; `roughjs`/`rough-notation` remain intentionally absent. The public/channel interaction shells and non-payment admin shell may retain their light comic character, but on 2026-07-14 the owner explicitly replaced the player's doodle experiment — first with a clean application-style treatment, then re-skinned the same custom controls to a "liquid glass" aesthetic (see the player-controls bullet below). Do not restore the old rough.js system or global paper utilities. `NajsIcon` remains unrelated shared icon infrastructure.
...
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx`), replacing Vidstack's stock `DefaultVideoLayout`. On 2026-07-14 the owner re-skinned them from the earlier flat blue-accent look to a **"liquid glass"** treatment (inspired by the Liquid-Glass-UI-Video-Player reference): the bottom bar is a single frosted-glass panel (`.polutek-glass--bar` — `backdrop-filter: blur()+saturate()`, layered `.polutek-glass-effect`/`.polutek-glass-noise` fractal-frost grain/`.polutek-glass-edge` inset "shine" highlight refracted by an inline `#polutek-liquid-glass` `feTurbulence`+`feDisplacementMap` SVG filter), with white (not blue) progress/volume fills, glass capsule sliders, a frosted-circle center hero/spinner and a dark-glass settings flyout. They use Lucide icons, a deliberate replay state, 40 px desktop / 44 px mobile targets, and a 6 px desktop / 6 px mobile progress track inside a larger scrub hit area. Visible ±10-second seek buttons and Picture-in-Picture are intentionally absent. Do not revert to the flat application-style player skin or restore the doodle experiment unless the owner asks. `PlayerLoadingState.tsx`, `AccessLockOverlay.tsx`, `PlayerErrorOverlay.tsx` and `PlayerStateFrame.tsx` own matching application-style player states; do not restore either the former black loading bar or the doodle player experiment. Player control CSS stays embedded via `<style jsx global>` inside `PolutekControls.tsx`; loading CSS stays beside `PlayerLoadingIndicator`. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]` rules are leftover from the old default skin.
