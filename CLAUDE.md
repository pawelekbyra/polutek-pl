...
**Update 2026-09-20 (TEMPORARY — test-mode UI changes, revert before real launch):** at the owner's explicit request, marked in the request itself as temporary and test-only ("to sa zmiany tymczasowe... żeby przyszłe AI mogło to cofnąć"). Three flags/overrides were added purely to make manual testing easier; none of them touch data, access control, or payments, and all three are independently revertible one-liners:
- `app/components/Navbar.tsx`: two module-level `const TEMP_HIDE_LANGUAGE_SWITCHER = true` / `const TEMP_HIDE_NOTIFICATIONS_BELL = true` flags. The language-switcher pill's JSX is wrapped in `{!TEMP_HIDE_LANGUAGE_SWITCHER && (...)}`; the signed-in `<NotificationsMenu />` (the "dzwoneczek"/bell) render is wrapped in `TEMP_HIDE_NOTIFICATIONS_BELL ? null : (...)` inside the existing `isSignedIn` branch (the `isLoaded`-signed-out and skeleton branches are untouched). **To revert:** flip both constants to `false`, or delete the flags and the two wrapping conditionals so the JSX renders unconditionally again exactly as before.
- `lib/i18n/server-language.ts`: a module-level `const TEMP_FORCE_ENGLISH = true` with an early `if (TEMP_FORCE_ENGLISH) return "en";` at the top of `resolveInitialLanguage()`, before the DB/cookie/geolocation/Accept-Language chain described in the function's own docstring (§4.11 of this file). This makes the root `/` redirect (`app/page.tsx`) always land on `/en`, and makes every fresh server-rendered page default to English, while leaving the underlying 5-step resolution logic completely intact and unreached. **To revert:** flip the constant to `false`, or delete the flag and the early-return line — the original priority chain immediately below it is untouched and needs no other changes.
- These three flags are the *only* changes in this update; do not extend "temporary test mode" to any other component without a fresh explicit request, and do not let this pattern (a `TEMP_*` boolean short-circuiting a component or function) become a general convention — it exists here only because the owner explicitly asked for an easy, obviously-temporary, single-file revert.
