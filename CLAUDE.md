...
- The legacy paper/ink hand-drawn implementation (rough.js `Frame`/`HachureFill`/`NajsSeparator`, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13; `roughjs`/`rough-notation` remain intentionally absent. The public/channel interaction shells and non-payment admin shell may retain their light comic character, but on 2026-07-14 the owner explicitly replaced the player's doodle experiment with a clean application-style treatment. Do not restore the old rough.js system or global paper utilities. `NajsIcon` remains unrelated shared icon infrastructure.
- The homepage channel surface (`Navbar.tsx`, `Hero.tsx`, `ChannelHome.tsx`, `SidebarPlaylist.tsx`, `DonationBox.tsx`/`DonationAmountField.tsx`, `AccessLockOverlay.tsx`, `Footer.tsx`, plus `SubscribeButton.tsx`/`ShareButton.tsx`/`InstallAppMenu.tsx`, which are flat-only — the old `colorScheme`/paper variant on `SubscribeButton.tsx` was dead code and was removed 2026-07-13) uses a separate flat, card-based, blue-accent (`#2563EB`) design skin — Space Grotesk (`font-brand`) headings, Plus Jakarta Sans (`font-sans`) body, one uniform page background (`--chan-nav`) shared by Navbar/content/Footer (no floating card / no separate dark canvas — that was tried and reverted per owner feedback). `.comments-flat-shell` (globals.css) re-skins the shared comments module for this surface. As of 2026-07-14, this surface also shares one radius/shadow token scale (`--chan-radius-sm/md/lg/xl` and `--chan-shadow-sm/md/lg/action`, defined in `globals.css` next to the `--chan-*` color tokens) so every card, pill, chip and button on the homepage picks a shared size instead of a one-off px/box-shadow value — this replaced a prior state where nearly every component (Hero, ChannelHome cards, SidebarPlaylist rows, DonationBox/DonationAmountField, AccessLockOverlay, Navbar buttons) had a distinct radius and shadow recipe. New homepage-surface components should reuse these tokens rather than inventing new arbitrary values; `/testujemy` and `/cyberpunk-light` only override the color `--chan-*` tokens per scope, so the radius/shadow tokens (unscoped, defined once in `:root`) apply unchanged across all three skins by design. As of 2026-07-14, `DonationBox.tsx`'s internal layout was also restructured into a HeroUI-Modal-like header/body/footer padding rhythm (`px-6` horizontal, `py-4` header, `py-2` body, `pb-4 pt-1` footer) in place of a hand-tuned negative-margin hack, and `AccessLockOverlay.tsx`'s full-screen (`default` variant) backdrop switched from a solid `--chan-nav` fill to a translucent `backdrop-blur-md backdrop-saturate-150` treatment, matching HeroUI Modal's `backdrop="blur"` pattern while keeping our tokens/colors. `DonationLegalDialog.tsx` (the one real modal on this surface, built on the shared `components/ui/dialog.tsx` primitive) picked up the same blurred-backdrop overlay via a new optional `overlayClassName` prop on `DialogContent`, plus `--chan-*`-token colors/radius/shadow in place of hardcoded hex grays. The shared `DialogContent` close button is now `rounded-full` globally (a small, backward-compatible tweak affecting every dialog, not just this surface).
...
- Two more `/testujemy`-style experimental re-skins exist, both `noindex`, both zero component duplication: `/cyberpunk` (`app/cyberpunk/page.tsx`) is a dark neon cyan/magenta re-skin scoped under `.cyberpunk-dark-page` in `app/cyberpunk.css` (imported once, unconditionally, from `app/layout.tsx` — every selector inside must stay prefixed with `.cyberpunk-dark-page`, or it leaks onto every other route); `/cyberpunk-light` (`app/cyberpunk-light/page.tsx`) keeps the real homepage's accent blue and light background and instead reads as cyberpunk through monospace/bracketed nav labels (`[ SZUKAJ ]`), a hairline grid background, glowing HUD corner brackets on the navbar and video frame, and a slow scan-line sweep, scoped under `.cyberpunk-light-page` in `app/globals.css` next to `.testujemy-page`. Do not make either theme the site's default again — `app/layout.tsx`/`app/globals.css`/`public/manifest.json` intentionally ship the normal light/blue baseline (`themeColor`/`manifest.json` `theme_color`/`background_color` are `#f7f1e4`); a prior iteration loaded the dark theme unconditionally for every visitor and that was reverted per owner feedback.
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx`), replacing Vidstack's stock `DefaultVideoLayout`. They use Lucide icons and the homepage's clean blue-accent application language, with regular geometry, restrained glass overlays, a deliberate replay state, 40 px desktop / 44 px mobile targets, and a 4 px desktop / 5 px mobile progress track inside a larger scrub hit area. Visible ±10-second seek buttons and Picture-in-Picture are intentionally absent (confirmed again 2026-07-14 — do not add them, even if a reference design like media-chrome shows them). `PlayerLoadingState.tsx`, `AccessLockOverlay.tsx`, `PlayerErrorOverlay.tsx` and `PlayerStateFrame.tsx` own matching application-style player states; do not restore either the former black loading bar or the doodle player experiment. Player control CSS stays embedded via `<style jsx global>` inside `PolutekControls.tsx`; loading CSS stays beside `PlayerLoadingIndicator`. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]` rules are leftover from the old default skin. On 2026-07-14 the bottom bar's visual language was adapted from media-chrome's default flat-toolbar theme (docked, edge-to-edge translucent dark bar instead of a floating pill row on a gradient scrim; square 0-radius buttons with a blue-tinted hover fill instead of rounded white-tint hover; a near-square 2 px-radius progress/volume track instead of a pill track; a dark flat settings popover with blue-tinted item hover instead of the previous cream/light popover) while keeping the blue accent, Lucide icons, and the existing feature set — this was a styling-only change, not a feature port from media-chrome.
- Cross-surface doodle styling is intentionally scoped rather than global: `.public-visual-shell` and `.channel-page-shell` cover public feedback/skeletons, while `AdminVisualShell` adds `.admin-visual-shell` to admin routes. `/admin/payments` is explicitly emitted as `.admin-visual-shell--payments` and must retain its existing approved visual treatment. Shared shadcn primitives expose `data-slot` hooks used by this scoped admin skin.
