...
- The legacy paper/ink hand-drawn visual system (rough.js `Frame`/`HachureFill`/`NajsSeparator`, Kalam/Patrick Hand fonts, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13 — it turned out to be dead everywhere except `app/error.tsx`, `app/not-found.tsx`, the homepage's error/empty fallback heading, and the `VideoPlayer` watermark, all now converted to the neutral shadcn base theme. The base shadcn theme tokens (`--background`/`--card`/`--popover`/`--secondary`, previously hardcoded to paper/beige colors) were retheme'd to a neutral white/gray palette, since `app/admin/**` and other shared shadcn UI (dialogs, popovers) read those tokens directly. `NajsIcon` (hand-drawn-style icon set, same file `app/components/najs/primitives.tsx`) is unrelated shared infra and was kept. `roughjs`/`rough-notation` were dropped from `package.json` as unused. Do not reintroduce this system without a fresh design decision.
...
- Two more `noindex` experimental routes, both pure additive CSS polish on top of main's *existing* colors/tokens (no palette swap, unlike the cyberpunk variants): `/polysk1` (`app/polysk1/page.tsx`, `.polysk1-page` in `globals.css`) is a "frosted glass" pass — translucent `backdrop-filter` panels on the nav/cards, a soft top sheen highlight, glassy buttons with a diagonal shine sweep on hover, rounder corners; `/polysk2` (`app/polysk2/page.tsx`, `.polysk2-page`) is a richer "glossy lacquer" pass — deeper elevation shadows, a bright specular highlight band across the top of cards/buttons, a beveled accent button, a soft contact-shadow under the video frame. Both target the same handful of selectors (`.polutek-watch-nav`, `[class*="bg-[var(--chan-card)]"]`/`[class*="bg-[var(--chan-surface)]"]`, `.bg-\[\#2563EB\]`, `[class*="rounded-*"]`, `.polutek-vidstack-player`, `footer`) — extend that same selector list in both blocks if a new card/button surface needs the same polish.
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx` + `app/components/player/icons.tsx`, PR #1422), replacing Vidstack's stock `DefaultVideoLayout`. Unlike the rest of the app's CSS (which lives in `app/globals.css`), this component's styling is embedded via `<style jsx global>` inside `PolutekControls.tsx` itself — check there (not `globals.css`) before assuming a `.polutek-player-*` class is unstyled. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]`-style rules are leftover dead CSS from the old `DefaultVideoLayout` skin.
