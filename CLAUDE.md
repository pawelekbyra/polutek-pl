...
- The homepage channel surface (`Navbar.tsx`, `Hero.tsx`, `ChannelHome.tsx`, `SidebarPlaylist.tsx`, `DonationBox.tsx`/`DonationAmountField.tsx`, `AccessLockOverlay.tsx`, `Footer.tsx`, plus `SubscribeButton.tsx`/`ShareButton.tsx`/`InstallAppMenu.tsx`, which are flat-only — the old `colorScheme`/paper variant on `SubscribeButton.tsx` was dead code and was removed 2026-07-13) uses a separate flat, card-based, blue-accent (`#2563EB`) design skin — Space Grotesk (`font-brand`) headings, Plus Jakarta Sans (`font-sans`) body, one uniform page background (`--chan-nav`) shared by Navbar/content/Footer (no floating card / no separate dark canvas — that was tried and reverted per owner feedback). `.comments-flat-shell` (globals.css) re-skins the shared comments module for this surface. On 2026-07-14 the homepage skin was restyled toward the neutral, card-based look of https://ant.design/components/overview while keeping every accent/color token unchanged (`--chan-blue: #2563eb` etc.): the `--chan-*` neutral tokens moved from warm cream `oklch(...)` values to true neutral grays (`--chan-nav: #f5f5f5`, `--chan-card: #ffffff`, `--chan-line: #f0f0f0`, `--chan-ink: #1f1f1f`, `--chan-muted: #8c8c8c`), new shared tokens `--chan-radius-sm/--chan-radius/--chan-radius-lg` (6/8/12px) and `--chan-shadow-card` (a subtle Ant-Card-style shadow) were added, and the previously inconsistent large/asymmetric card radii (`rounded-[22px]`/`rounded-[24px]`/hard offset "comic" box-shadows/rotate transforms) across `Navbar.tsx`, `Hero.tsx`, `ChannelHome.tsx`, `SidebarPlaylist.tsx`, `DonationBox.tsx`, `SubscribeButton.tsx`, `ShareButton.tsx` and `AccessLockOverlay.tsx` were normalized to the small consistent radii above. Do not reintroduce the warm cream `--chan-*` values, the navbar's radial-gradient/blur background, or hard offset button shadows on this surface without owner sign-off — `/channel/[slug]` (`channel-page-shell`) and the admin shell intentionally keep their own separate comic character and were not touched by this pass.
