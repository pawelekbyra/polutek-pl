...
- The legacy paper/ink hand-drawn implementation (rough.js `Frame`/`HachureFill`/`NajsSeparator`, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13; `roughjs`/`rough-notation` remain intentionally absent. On 2026-07-14 the owner made a fresh design decision to use a lighter doodle/comic character again, implemented with CSS and inline SVG only: player controls/loading, the public/channel interaction shells, and the non-payment admin shell. Do not restore the old rough.js system or global paper utilities. `NajsIcon` remains unrelated shared icon infrastructure.
...
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx` + `app/components/player/icons.tsx`), replacing Vidstack's stock `DefaultVideoLayout`. The controls use scoped doodle/comic styling, a deliberate replay end-state, 42 px desktop / 44 px mobile targets with smaller visual button bodies, and a 4 px desktop / 5 px mobile progress track inside a larger scrub hit area. `PlayerLoadingState.tsx` owns the matching warm doodle loading state; do not restore the former black bar. Player control CSS stays embedded via `<style jsx global>` inside `PolutekControls.tsx`; loading CSS stays beside `PlayerLoadingIndicator`. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]` rules are leftover from the old default skin.
