...
- As of 2026-09-08, the site logo (`app/components/BrandName.tsx`, used by `Navbar.tsx`) is a **"KUTASHI.COM" text wordmark** — a `<span>` set in the Space Grotesk Bold `font-brandLogo`/`--font-brand-logo` font (`app/fonts/space-grotesk-bold-latin.woff2`, already wired into `app/layout.tsx`/`tailwind.config.ts`), `KUTASHI` in `--chan-ink` and the `.COM` suffix picked out in `--chan-amber` to match the site's blue/amber accent pairing — replacing the 2026-07-30 glasses/sunglasses graphic (`public/logo-glasses.svg`). This is an owner-directed rename of the wordmark only: it does not touch `APP_NAME`/`APP_DOMAIN` in `lib/constants.ts`, page titles/metadata, the PWA manifest, legal-doc owner lines, the PWA/favicon app icon (`app/icon.tsx`/`app/icon-512/route.tsx`, still the glasses mark via `lib/icons/app-icon.ts`'s `GLASSES_MARK_DATA_URI`), or any DNS/Clerk hostnames — only broaden this rename beyond the navbar wordmark on an explicit request, the same caution as the POLUTEK→PAWELPERFECT rebrand above. `BrandName` keeps the same `className`/`decorative` props as the glasses-graphic version, but `className` now controls **font-size** (e.g. `text-[20px]`), not height — the old `h-*`/`w-auto` sizing convention doesn't apply to a text node. `decorative` renders `aria-hidden="true"` with no `aria-label` (there's no `alt` on a `<span>`) for the case where an enclosing `<Link>` already carries its own `aria-label="KUTASHI.COM"`, avoiding double-announcing the name to screen readers; the non-decorative default exposes the mark as `role="img" aria-label="KUTASHI.COM"` so assistive tech still gets one clean accessible name rather than reading "KUTASHI" and ".COM" as two words. Logo size is `text-[20px] md:text-[24px]` in the Navbar's `h-[38px]` slot. `CheckoutSummaryPanel.tsx` does not currently render `BrandName` (a stale claim from an earlier revision of this file) — check before assuming it needs the same treatment. The Navbar's sign-in/"ENTER" button (`Navbar.tsx`, `t.signIn`) is a fixed `w-[38px]` icon-only circle below `sm:` (matching the other 38×38 icon buttons already in the navbar, and roughly one language-switcher segment) instead of `px-3.5` padding around a hidden-on-mobile text label, which had made it visibly larger than its neighbors; it only grows to fit the "ENTER" text at `sm:` and up (`sm:w-auto sm:px-4`).
