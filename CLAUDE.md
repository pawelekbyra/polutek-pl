...
- Public typography is self-hosted Geist Sans/Mono from the `geist` package. `font-sans`, `font-heading`, and `font-brand` intentionally resolve to Geist Sans; `font-mono` resolves to Geist Mono. Builds must not fetch remote fonts. The `POLUTEK.PL` wordmark (`app/components/BrandName.tsx`, `font-brandLogo`/`--font-brand-logo`) is set in Frijole, self-hosted from `app/fonts/frijole-latin.woff2` (OFL license alongside it) — chosen 2026-07-15 from the temporary `/logoN` font bake-off, which has since been deleted along with its `logo-experiment` components and `/public/fonts/logo/*` assets. "POLUTEK" stays `--chan-ink`; the ".PL" suffix stays `--chan-blue` — keep that split when touching the wordmark.
