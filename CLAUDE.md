...
- The homepage channel surface (`Navbar.tsx`, `Hero.tsx`, `ChannelHome.tsx`, `SidebarPlaylist.tsx`, `DonationBox.tsx`/`DonationAmountField.tsx`, `AccessLockOverlay.tsx`, `Footer.tsx`, plus `SubscribeButton.tsx`/`ShareButton.tsx`/`InstallAppMenu.tsx`, which are flat-only — the old `colorScheme`/paper variant on `SubscribeButton.tsx` was dead code and was removed 2026-07-13) uses a separate flat, card-based, blue-accent (`#2563EB`) design skin — Space Grotesk (`font-brand`) headings, Plus Jakarta Sans (`font-sans`) body, one uniform page background (`--chan-nav`) shared by Navbar/content/Footer (no floating card / no separate dark canvas — that was tried and reverted per owner feedback). `.comments-flat-shell` (globals.css) re-skins the shared comments module for this surface.
- As of 2026-07-14 the homepage channel surface is built on `flowbite-react` (installed as a dependency): `Navbar`/`NavbarBrand`, `TextInput`, `Button`/`ButtonGroup`, `Badge`, `Card`, and `Footer`/`FooterCopyright`/`FooterLinkGroup`/`FooterLink` replace the equivalent hand-rolled markup in `Navbar.tsx`, `Hero.tsx`, `Footer.tsx`, `ChannelHome.tsx`, `SidebarPlaylist.tsx`, and `DonationBox.tsx`, restyled to the existing flat blue-accent (`#2563EB`) palette rather than Flowbite's own defaults. Business logic, state, and data wiring in those files are unchanged — only the rendered markup/classNames moved onto Flowbite primitives. `.flowbite-react/` (generated `class-list.json`/`config.json`/`init.tsx`, dark mode disabled since this product has no dark theme) and the `@plugin`/`@source` directives in `app/globals.css` plus `withFlowbiteReact` in `next.config.mjs` are required wiring — do not remove them or the Tailwind v4 `@source` scan of Flowbite's dynamic classes will break. `<ThemeInit />` is mounted in `app/layout.tsx`'s `<head>`; it is required by the flowbite-react runtime, not decorative. When touching these files, prefer extending with more Flowbite primitives (`Avatar`, `Dropdown`, `Tabs`, etc.) over reintroducing bespoke divs/buttons, to keep the surface visually consistent.
