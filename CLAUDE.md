...
- Video player controls remain fully custom (`app/components/player/PolutekControls.tsx`) rather than Vidstack's stock layout. `PlayerLoadingState`, `AccessLockOverlay`, `PlayerErrorOverlay` and `PlayerStateFrame` own matching player states. The full login-required and patron-required locks are intentionally distinct, friendly illustrated React surfaces (blue ticket/seating vs yellow-orange Thank You Zone), not recolors of one corporate dark panel; patron styling remains visible even when a guest must sign in first. Compact thumbnail locks stay static, all motion must stop under `prefers-reduced-motion`, and denied states must never keep a player mounted for an exit animation.
