...
- Video player controls remain fully custom (`app/components/player/PolutekControls.tsx`) rather than Vidstack's stock layout. `PlayerLoadingState`, `AccessLockOverlay`, `PlayerErrorOverlay` and `PlayerStateFrame` own matching player states. The full login-required and patron-required locks are intentionally distinct, friendly illustrated React surfaces (blue ticket/seating vs yellow-orange Thank You Zone), not recolors of one corporate dark panel; patron styling remains visible even when a guest must sign in first. As of 2026-07-14 the two scenes (`app/components/AccessLockOverlay.module.css`) use **darker, saturated accent-color gradients** (deep `--chan-blue` for login, deep `--chan-amber`/`--chan-amber-strong` for patron) with white/cream text and inverted white CTA buttons — deliberately reverted from an earlier light-pastel treatment the owner found too washed out. Keep both scenes clearly darker than a pastel tint and rooted in the site's accent button colors; don't re-lighten the backgrounds to soft tints without an explicit request. Compact thumbnail locks stay static, all motion must stop under `prefers-reduced-motion`, and denied states must never keep a player mounted for an exit animation.
