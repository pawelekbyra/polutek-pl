...
**Update 2026-09-20 (Regulamin/Terms domain reference → kutashi.com):** at the owner's explicit request, the site-address reference inside `RegulaminContent`'s and `TermsContentEn`'s `leadIn` strings (the two lines quoted just above) was changed from `www.pawelperfect.pl` to `www.kutashi.com`, and the static footer mark (`app/components/Footer.tsx`) was updated the same way. This was scoped narrowly to exactly those two `leadIn` strings and the footer text — **`PolitykaContent`'s PL leadIn ("Administratorem serwisu www.pawelperfect.pl jest") was deliberately left unchanged**, since the request named only "regulamin" (Terms of Service), not Polityka Prywatności (Privacy Policy) — these are two distinct documents in this codebase's own terminology. Do not "fix" that asymmetry by also changing Polityka's domain reference without a separate explicit request; conversely, do not assume `www.kutashi.com` should propagate to the Clerk/CSP hostnames in `lib/utils/security.ts`, `MAIN_CREATOR_SLUG`, `NEXT_PUBLIC_APP_URL`, or `EMAIL_FROM` — those remain real DNS/Clerk custom-domain/DB-lookup-key values untouched by this change, per the same reasoning already documented in the 2026-07-30 rebrand section above (this is a legal-document/footer content edit, not an infrastructure cutover). **Follow-up (same day):** the first pass missed that `app/[locale]/regulamin/page.tsx` and `app/[locale]/terms/page.tsx` (the route wrapper pages, not `LegalDocs.tsx`) separately render their own page `<title>`/`<h1>` using the `APP_NAME` brand constant ("Regulamin serwisu {APP_NAME}" / "Terms of Service — {APP_NAME}"), which still resolved to the PAWELPERFECT.PL brand name — a distinct string from the `www.pawelperfect.pl` domain reference already fixed above, easy to miss since it's brand name vs. domain, not the same literal string. Both page files now hardcode `www.kutashi.com` directly in `metadata.title`, `metadata.description`, and the visible `<h1>`, and no longer import `APP_NAME` at all. `app/[locale]/polityka-prywatnosci/page.tsx` and `app/[locale]/privacy-policy/page.tsx` use the identical `{APP_NAME}`-in-title/h1 pattern and were deliberately left alone, matching the Polityka/Privacy exclusion above — don't "fix" those either without a separate explicit request.
