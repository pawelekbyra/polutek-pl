...
- Video player controls remain fully custom (`app/components/player/PolutekControls.tsx`) rather than Vidstack's stock layout. `PlayerLoadingState`, `AccessLockOverlay`, `PlayerErrorOverlay` and `PlayerStateFrame` own matching player states. As of 2026-07-14 the full login-required and patron-required locks (`app/components/AccessLockOverlay.tsx` / `.module.css`) are a near-black (`#0a0a0a`) "dark corporate panel" style, per an explicit owner request that reverted an earlier light/illustrated treatment: a blurred colored `.glow` layer (deep `--chan-blue` for login, deep `--chan-amber`/`--chan-amber-strong` for patron) plus a subtle grain `.noise` overlay behind a centered `Lock`/`Gem` icon, a bold two-line uppercase headline (white + accent-colored word), a thin divider, and a minimal text-only CTA (thin line + widely tracked label) that brightens on hover/focus — no card, no button chrome. Compact thumbnail locks use the same near-black glow treatment at a smaller scale. Don't reintroduce the earlier pastel/illustrated ticket-and-thank-you-card design or add card/button chrome back without an explicit request; patron styling remains visible even when a guest must sign in first. All motion must stop under `prefers-reduced-motion`, and denied states must never keep a player mounted for an exit animation.
