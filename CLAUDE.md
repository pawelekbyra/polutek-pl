...
- Public typography is self-hosted Geist Sans/Mono from the `geist` package. `font-sans`, `font-heading`, and `font-brand` intentionally resolve to Geist Sans; `font-mono` resolves to Geist Mono. Builds must not fetch remote fonts. The `POLUTEK.PL` wordmark (`app/components/BrandName.tsx`, `font-brandLogo`/`--font-brand-logo`) is set in Space Grotesk Bold, self-hosted from `app/fonts/space-grotesk-bold-latin.woff2` (OFL license alongside it). "POLUTEK" stays `--chan-ink`; the ".PL" suffix stays `--chan-blue` — keep that split when touching the wordmark. `BrandName` renders as a mark + wordmark lockup by default: an ink rounded-square carrying the same "enter" glyph used by the favicon/PWA icon (`enterGlyphFilledPath()` in `lib/icons/app-icon.ts`, the single source of truth for that glyph), in `--chan-blue`, sized in `em` so it scales with the surrounding font-size. Pass `mark={false}` only where the extra glyph doesn't fit (e.g. very tight inline contexts) — don't fork a second glyph shape.
