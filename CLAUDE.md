...
- The legacy paper/ink hand-drawn implementation (rough.js `Frame`/`HachureFill`/`NajsSeparator`, `--najs-*` tokens, `.paper-*`/`.ink-*` utility classes, `.comments-paper-shell`, `.ui-clone-skin`) was removed 2026-07-13; `roughjs`/`rough-notation` remain intentionally absent. The public/channel interaction shells and non-payment admin shell may retain their light comic character, but on 2026-07-14 the owner explicitly replaced the player's doodle experiment — cycling through three re-skins: application-style flat look, "liquid glass" frosted panels, Netflix minimalist, and finally a **Demuxed 2022 conference aesthetic** (see the player-controls bullet below). Do not restore the old rough.js system or global paper utilities. `NajsIcon` remains unrelated shared icon infrastructure.
...
- Video player controls are fully custom (`app/components/player/PolutekControls.tsx`), replacing Vidstack's stock `DefaultVideoLayout`. On 2026-07-14 the owner re-skinned them to a **Demuxed 2022 conference style** (inspired by the Demuxed 2022 theme from muxinc/player.style): the bottom control bar is a single pill-shaped panel (`border-radius:9999px`, `background:rgba(0,0,0,.2)`, `margin:30px`, `padding:10px 14px`) with white circular buttons (32px, `border-radius:9999px`); blue glow accent (`#7596CC`) on all hover states (`box-shadow: 0 0 0 2px`); white progress bar fill on semi-transparent dark track (`rgba(0,0,0,.4)`), blue thumb (`#7596CC`, 14px); large centered play button (96px, `background:rgba(0,0,0,.8)`) with blue glow on hover; vertical volume slider revealed on mute-button hover (rotated -90°); dark settings menu (`rgba(0,0,0,.9)`) with blue accent border; decorative gradient scrim at bottom. They use Lucide icons, a deliberate replay state, 32 px desktop / 32 px mobile targets, and a 6 px desktop / 5 px mobile progress track inside a 24 px hit area. Visible ±10-second seek buttons and Picture-in-Picture are intentionally absent. Do not restore the liquid glass, Netflix, or doodle styles unless the owner asks. `PlayerLoadingState.tsx`, `AccessLockOverlay.tsx`, `PlayerErrorOverlay.tsx` and `PlayerStateFrame.tsx` own matching application-style player states; do not restore either the former black loading bar or the doodle player experiment. Player control CSS stays embedded via `<style jsx global>` inside `PolutekControls.tsx`; loading CSS stays beside `PlayerLoadingIndicator`. `globals.css`'s `.polutek-vidstack-player [class*="vds-slider-fill"]` rules are leftover from the old default skin.
