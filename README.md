...
- [x] Znaleziono i naprawiono niespójność CSP: usunięto szerokie `https://*.r2.dev`, `https://*.vercel-storage.com` z `Content-Security-Policy-Report-Only`; CSP korzysta teraz z dokładnych hostów ENV dla mediów/obrazów, a wyjątki Google Fonts są jawne dla przywróconych fontów UI; jest regresja `tests/unit/csp.test.ts`.
...
- [x] Unit suite PASS: 27 plików, 139 testów.
