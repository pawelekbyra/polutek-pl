...
type SkinComponent = React.ComponentType<{ children: React.ReactNode }>;
...
export default async function ExperimentHomePage({ Skin, searchParams, loggerPrefix }: ExperimentHomePageProps) {
  const params = await searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (error) {
      logger.error(`[${loggerPrefix}_AUTH_ERROR]`, error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error(`[${loggerPrefix}_CURRENT_USER_ERROR]`, error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error(`[${loggerPrefix}_USER_FETCH_ERROR]`, error);
    });
