...
const doodleIconClass =
  "h-5 w-5 drop-shadow-[1.5px_1.5px_0_rgba(14,165,233,0.45)]";
...
function DoodlePlayIcon() {
  return (
    <svg viewBox="0 0 24 24" className={doodleIconClass} aria-hidden="true">
      <path
        d="M8.2 5.4c-.9.5-1.1 11.8-.1 12.7.8.8 10.1-4.7 10.3-6 .2-1.3-9.2-7.3-10.2-6.7Z"
        fill="currentColor"
        stroke="white"
        strokeWidth="1.7"
...
function DoodlePauseIcon() {
  return (
    <svg viewBox="0 0 24 24" className={doodleIconClass} aria-hidden="true">
      <path
        d="M8 5.2c-1.2.1-1.4.8-1.4 6.9 0 5.8.3 6.7 1.5 6.8l2.2.1c1.1-.1 1.4-.9 1.4-6.9 0-6.2-.4-6.9-1.5-7L8 5.2Zm7.1-.1c-1.1.1-1.4.9-1.4 7 0 5.9.3 6.7 1.5 6.8l2 .1c1.2-.1 1.5-.9 1.5-6.9 0-6.1-.3-6.8-1.4-7l-2.2 0Z"
        fill="currentColor"
        stroke="white"
        strokeWidth="1.35"
        strokeLinejoin="round"
...
function DoodleVolumeIcon() {
  return (
    <svg viewBox="0 0 24 24" className={doodleIconClass} aria-hidden="true">
      <path
        d="M4.2 9.1h3.2l4.1-3.2c.8-.6 1.9-.1 1.9.9v10.4c0 1-1.1 1.5-1.9.9l-4.1-3.2H4.2c-.8 0-1.4-.6-1.4-1.4v-3c0-.8.6-1.4 1.4-1.4Z"
        fill="currentColor"
        stroke="white"
        strokeWidth="1.35"
        strokeLinejoin="round"
      />
      <path
        d="M16.1 8.3c1.7 1.8 1.8 5.3 0 7.3M18.7 6.1c3.1 3.1 3.2 8.4.1 11.7"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
...
function DoodleCaptionsIcon() {
  return (
    <svg viewBox="0 0 24 24" className={doodleIconClass} aria-hidden="true">
      <path
        d="M4.2 6.2c1.4-1 14.3-.9 15.5.1 1.2.9 1.1 10.4-.1 11.3-1.5 1-13.9 1.2-15.3.1-1.2-1-1.4-10.5-.1-11.5Z"
        fill="currentColor"
        stroke="white"
        strokeWidth="1.5"
        strokeLinejoin="round"
      />
      <path
        d="M7.1 11.1h3.4M13.5 11.1h3.4M7.1 14.6h5.2M15 14.6h2"
        stroke="white"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
...
function DoodleFullscreenIcon() {
  return (
    <svg viewBox="0 0 24 24" className={doodleIconClass} aria-hidden="true">
      <path
        d="M5.4 9V5.4H9M15 5.4h3.6V9M18.6 15v3.6H15M9 18.6H5.4V15"
        fill="none"
        stroke="currentColor"
        strokeWidth="2.4"
        strokeLinecap="round"
      ...
      />
      <path
        d="M6 6l4 4M18 6l-4 4M18 18l-4-4M6 18l4-4"
        stroke="white"
        strokeWidth="1.2"
        strokeLinecap="round"
      />
...
function DoodlePlayerControls() {
  const buttonClass =
    "grid h-10 w-10 place-items-center rounded-2xl border border-white/15 bg-white/10 text-white transition hover:-translate-y-0.5 hover:bg-sky-500 hover:shadow-[2px_3px_0_rgba(255,255,255,0.18)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-200";
  ...
  return (
    <Controls.Root className="absolute inset-0 z-30 flex flex-col justify-end bg-gradient-to-t from-black/82 via-black/24 to-black/10 opacity-0 transition-opacity duration-200 group-hover:opacity-100 data-[visible]:opacity-100">
      <div className="px-2 pb-1 sm:px-4 sm:pb-2">
        <TimeSlider.Root
          className="group/slider relative mb-1 flex h-7 w-full cursor-pointer touch-none select-none items-center"
        ...
        <Controls.Group className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <VolumeSlider.Root
              className="group/volume hidden h-10 w-24 items-center md:flex"
              aria-label="Głośność"
            >
            ...
            <CaptionButton className={buttonClass} aria-label="Napisy">
              <DoodleCaptionsIcon />
            </CaptionButton>
          </div>
          <div className="flex flex-col items-end gap-1 text-right">
            <div className="rounded-full border border-white/15 bg-black/35 px-3 py-1 text-[11px] font-bold tabular-nums tracking-wide text-white shadow-[2px_2px_0_rgba(14,165,233,0.26)] sm:text-xs">
              <Time type="current" /> <span className="text-white/55">/</span>{" "}
              <Time type="duration" />
            </div>
            <FullscreenButton className={buttonClass} aria-label="Pełny ekran">
              <DoodleFullscreenIcon />
            </FullscreenButton>
          </div>
        </Controls.Group>
      </div>
...
export default function VideoPlayer({
  ...
  const cloudflareViewFallbackTimer = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  const reachedThresholds = useRef<Record<number, boolean>>({});

  const sendEvent = useCallback(
    async (type: string, extra = {}) => {
      ...
      try {
        const res = await fetch(`/api/videos/${video.id}/playback-event`, {
          ...
          body: JSON.stringify({
            ...
            type,
            positionMs: player.current
              ? Math.floor(player.current.currentTime * 1000)
              : 0,
            durationMs: player.current
              ? Math.floor(player.current.duration * 1000)
              : 0,
            ...extra,
          }),
        });
  ...
  const scheduleCloudflareViewFallback = useCallback(() => {
    ...
    cloudflareViewFallbackTimer.current = setTimeout(
      sendWatched10Seconds,
      10000,
    );
        ...
        {loadError ? (
          ...
          <iframe
            ...
            onError={() => {
              clearCloudflareViewFallback();
              setLoadError(
                "Nie udało się załadować materiału wideo. Sprawdź dostępność źródła.",
              );
              setPlayerKey((k) => k + 1);
    ...
    <div className="relative w-full h-full min-h-0 sm:min-h-[220px] bg-black rounded-xl overflow-hidden shadow-2xl group">
      ...
      {loadError ? (
        ...
        <MediaPlayer
          ...
          onSeeked={(e: any) =>
            sendEvent("SEEKED", { positionMs: Math.floor(e.detail * 1000) })
          }
