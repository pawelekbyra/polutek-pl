...
type GameState = 'IDLE' | 'SHOW_BALL' | 'COVER_BALL' | 'SHUFFLING' | 'WAITING_FOR_GUESS' | 'REVEAL_WRONG' | 'REVEAL_RIGHT' | 'RESULT';
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md overflow-hidden">
      <motion.div
        ...
        className="relative w-full max-w-2xl rounded-[2.5rem] border border-white/10 bg-gradient-to-b from-neutral-900 to-black p-8 text-center shadow-2xl overflow-hidden"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="relative z-10 mb-12">
            <motion.p
                ...
                className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-2"
            >
                {t.tagline}
            </motion.p>
            <motion.h3
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-4xl font-brand font-black tracking-tighter text-white"
            >
                {t.title}
            </motion.h3>
        ...
        <div className="relative h-64 w-full flex items-end justify-center gap-4 px-4 pb-8 perspective-1000">
            ...
            {/* Ball */}
            <AnimatePresence>
                {(gameState === 'SHOW_BALL' || gameState === 'REVEAL_RIGHT' || gameState === 'RESULT') && (
                    <motion.div
                        initial={{ opacity: 0, scale: 0 }}
                        animate={{
                            opacity: 1,
                            scale: 1,
                            x: (cupPositions[realBallCup ?? ballPosition] - 1) * 160 // Center is 1
                        }}
                        exit={{ opacity: 0, scale: 0 }}
                        transition={{ type: "spring", damping: 20, stiffness: 150 }}
                        className="absolute bottom-8 left-1/2 -translate-x-1/2 w-6 h-6 rounded-full bg-white shadow-[0_0_15px_rgba(255,255,255,0.8)] z-0"
                    >
            ...
            {[0, 1, 2].map((cupIdx) => {
                ...
                const isRevealed = (isSelected && gameState === 'REVEAL_WRONG') ||
                ...
                return (
                    <motion.div
                        ...
                        animate={{
                            x: (currentSlot - 1) * 160,
                            y: isRevealed ? -100 : 0,
                            rotateX: isRevealed ? -15 : 0,
                            cursor: gameState === 'WAITING_FOR_GUESS' ? 'pointer' : 'default'
                        }}
                    ...
                    >
                        <div className="relative">
                            ...
                            <div className={cn(
                                ...
                                isSelected && "ring-2 ring-primary ring-offset-4 ring-offset-black"
                            )}>
            ...
            <AnimatePresence mode="wait">
                {gameState === 'SHUFFLING' && (
                    ...
                    >
                        ...
                        <span className="font-bold uppercase tracking-widest text-xs">{t.shuffling}</span>
                    </motion.div>
                )}
                ...
                {gameState === 'RESULT' && (
                    ...
                    >
                        <div className="space-y-1">
                            <h4 className="text-primary font-black uppercase tracking-widest text-lg">{t.miss}</h4>
