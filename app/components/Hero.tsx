...
const Hero: React.FC<HeroProps> = ({
  ...
  const displayDescription =
    language === "en" && video.descriptionEn
      ? video.descriptionEn
  ...
  useEffect(() => {
    ...
    video.id,
    userId,
    initialInteraction?.liked,
    initialInteraction?.disliked,
    video.likesCount,
    video.dislikesCount,
  ]);
  ...
  const handleLike = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = (await toggleVideoLike(video.id)) as {
          liked: boolean;
          disliked: boolean;
          likesCount: number;
  ...
  const handleDislike = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = (await toggleVideoDislike(video.id)) as {
          liked: boolean;
          disliked: boolean;
          likesCount: number;
          dislikesCount: number;
  ...
  return (
    <section className="bg-transparent">
      <div className="w-full">
        {/* FEATURED MEDIA */}
        <div className="relative aspect-video w-full mb-[18px] group drop-shadow-[0_12px_22px_rgba(23,23,23,0.22)]">
          ...
          <div className="absolute inset-0 overflow-hidden rounded-[12px] bg-black">
            ...
            >
              <VideoPlayer
                video={video}
                onViewCounted={() => setLocalViewsCount((views) => views + 1)}
              />
            </PremiumWrapper>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h1
            className="font-bold text-[23px] text-[#0f0f0f] leading-[1.25] mb-[14px]"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-[14px]">
            <div className="flex w-full items-center gap-[13px] min-w-0 lg:w-auto">
              <Link
                href={
                  video.creator?.slug ? `/channel/${video.creator.slug}` : "#"
                }
              ...
              >
                <Image
                  src={
                    ...
                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${video.creator?.name || MAIN_CREATOR_NAME}`
                  }
              ...
              <div className="min-w-0 flex flex-col">
                ...
                <span className="text-[12.5px] text-muted-foreground mt-[1px]">
                  {mounted
                    ? formatCount(localSubState.subscribersCount)
                    : video.creator?.subscribersCount || 0}{" "}
                  {t.subscribers}
                </span>
              </div>
              ...
              <div className="relative flex h-[38px] flex-1 items-center lg:flex-none">
                ...
                >
                  <NajsIcon
                    ...
                    stroke={interactionState.isLiked ? "#2563eb" : INK}
                  />
                  <span
                ...
                >
                  <NajsIcon
                    name="dislike"
                    className="h-[18px] w-[18px]"
                    stroke={interactionState.isDisliked ? "#2563eb" : INK}
                  />
                  <span
                    className="text-[14px] font-bold"
                    style={{ fontFamily: "var(--font-najs, Kalam, cursive)" }}
        ...
        {/* DESCRIPTION BOX */}
        <div
          className="relative mt-[16px] p-[14px] cursor-pointer drop-shadow-[0_9px_16px_rgba(23,23,23,0.18)]"
        ...
        >
          <Frame
            ...
            stroke={INK}
            strokeWidth={1.2}
            fill="rgba(248,243,231,.95)"
          />
          <div className="relative z-10">
            <div className="flex flex-wrap gap-x-2 gap-y-0.5 mb-[7px] items-baseline">
              <span
                className="text-[13.5px] font-bold text-[#0f0f0f]"
                style={{ fontFamily: "var(--font-najs, Kalam, cursive)" }}
              >
                {mounted
                  ? localViewsCount.toLocaleString(
