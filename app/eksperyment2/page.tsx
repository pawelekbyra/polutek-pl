...
import E2Home from './E2Home';

export const dynamic = 'force-dynamic';

export default async function Eksperyment2(props: { searchParams: Promise<{ v?: string }> }) {
  const searchParams = await props.searchParams;

  const getSafeAuth = async () => {
    try { return await auth(); }
    catch (e) { logger.error("[EXP2_AUTH]", e); return { userId: null as string | null }; }
  };
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => logger.error("[EXP2_USER]", e));
