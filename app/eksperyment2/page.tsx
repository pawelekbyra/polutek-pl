...
import ExperimentTwoSkin from "./ExperimentTwoSkin";
...
export default async function Eksperyment2Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  ...
  const getSafeAuth = async () => {
    try {
      ...
      logger.error("[EXPERIMENT2_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT2_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT2_USER_FETCH_ERROR]", error);
    });
