...
import ExperimentFiveSkin from "./ExperimentFiveSkin";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Eksperyment 5 — nocne kino · Polutek",
  description: "Strona główna Polutek w neonowej, kinowej stylizacji VHS.",
};

export default async function Eksperyment5Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (error) {
      logger.error("[EXPERIMENT5_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT5_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT5_USER_FETCH_ERROR]", error);
    });
