...
export const metadata: Metadata = {
  title: `Testujemy — ${APP_NAME}`,
  description: 'Eksperymentalna, elegancka odsłona strony głównej — działająca kopia z innym stylem wizualnym.',
  robots: { index: false, follow: false },
};

/**
 * Design experiment: the exact same homepage data/logic/components as `/`
 * (Navbar, ChannelHome, Footer — same fetches, same auth, same like/subscribe/
 * comments/donation flows), wrapped in the `.testujemy-page` CSS scope from
 * globals.css, which re-themes those components into an elegant "maison
 * française" palette instead of the flat blue one. No component logic is
 * duplicated here — only the visual skin differs.
 */
export default async function TestujemyPage(props: { searchParams: Promise<{ v?: string, q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (e) {
      logger.error("[TESTUJEMY_AUTH_ERROR]", e);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((e) => {
      logger.error("[TESTUJEMY_CURRENT_USER_ERROR]", e);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => {
      logger.error("[TESTUJEMY_USER_FETCH_ERROR]", e);
    });
