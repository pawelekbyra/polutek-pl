...
import { approveHeldComment } from "@/lib/modules/comments";
...
export async function POST(request: NextRequest, props: { params: Promise<{ commentId: string }> }) {
  ...
  const { adminUserId, response } = await requireAdminForApi("APPROVE_COMMENT");
  if (response) return response;
  try {
    const actor = { type: "admin" as const, userId: adminUserId! };
    const result = await approveHeldComment(params.commentId, createAppContext({ actor }));
