...
export default async function Eksperyment1(props: { searchParams: Promise<{ v?: string }> }) {
  const searchParams = await props.searchParams;

  const getSafeAuth = async () => {
    try { return await auth(); }
    catch (e) { logger.error("[EXP1_AUTH_ERROR]", e); return { userId: null as string | null }; }
  };

  const [authData, content, user] = await Promise.all([
    getSafeAuth(),
    loadHomeContent(),
    currentUser().catch((e) => { logger.error("[EXP1_USER_ERROR]", e); return null; }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => { logger.error("[EXP1_USER_FETCH_ERROR]", e); });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" style={{ fontFamily: "var(--font-patrick, 'Patrick Hand', cursive)" }}>
      <Navbar />
