    ...
    <svg ref={ref} className="absolute inset-0 h-full w-full pointer-events-none" aria-hidden="true">
      {d && (
        <>
          <path d={d} fill={fill} />
          {shade && (
            <g opacity=".12" stroke={stroke} strokeWidth=".8">
              {Array.from({ length: 12 }).map((_, i) => (
                <path key={i} d={`M ${7 + i * 16} ${size.h - 7} L ${27 + i * 16} ${size.h - 21}`} />
              ))}
            </g>
          )}
...
function N4Highlight({ color, opacity = 0.7, seed = 1 }: { color: string; opacity?: number; seed?: number }) {
  const { ref, size } = useParentSize<SVGSVGElement>();
  const { w, h } = size;
  const d = w && h
    ? `M ${7 + wobble(seed, 1, 2)} ${8 + wobble(seed, 2, 2)} Q ${w / 2} ${2 + wobble(seed, 3, 2)} ${w - 7 + wobble(seed, 4, 2)} ${8 + wobble(seed, 5, 2)} L ${w - 8 + wobble(seed, 6, 2)} ${h - 7 + wobble(seed, 7, 2)} Q ${w / 2} ${h - 2 + wobble(seed, 8, 2)} ${7 + wobble(seed, 9, 2)} ${h - 8 + wobble(seed, 10, 2)} Z`
    : "";
...
function I({ name, className = "h-5 w-5", stroke = "currentColor" }: { name: Icon; className?: string; stroke?: string }) {
  ...
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden="true">
      ...
      {name === "pause" && <><path d="M8 6 v12" {...c} strokeWidth={4} /><path d="M16 6 v12" {...c} strokeWidth={4} /></>}
...
function Btn({ children, href, filled = false, icon, compact = false, className = "" }: { children?: React.ReactNode; href?: string; filled?: boolean; icon?: Icon; compact?: boolean; className?: string }) {
  const body = (
    <span
      className={`relative inline-flex min-h-9 items-center justify-center gap-2 px-3.5 text-sm font-black leading-none tracking-wide transition-transform hover:-translate-y-0.5 ${compact ? "min-w-9 px-2.5" : ""} ${className}`}
      style={{ color: filled ? "white" : INK }}
    >
      <Frame radius={18} seed={filled ? 41 : 39} stroke={INK} strokeWidth={1.2} fill={filled ? INK : "rgba(248,243,231,.88)"} shade={filled} />
      {icon && <I name={icon} className="relative h-5 w-5" />}
...
function SupportButton() {
  return (
    <button type="button" className="relative h-12 w-full overflow-hidden text-base font-black text-white transition-transform hover:-translate-y-0.5">
      <N4Highlight color="#4f83ff" opacity={0.86} seed={330} />
      <HachureFill seed={335} />
      <Frame radius={12} seed={333} stroke="#1748b8" strokeWidth={1.35} />
...
function Lang() {
  return (
    <div className="relative flex h-9 items-center px-1 text-sm font-black">
      <Frame radius={18} seed={52} stroke={INK} strokeWidth={1.1} fill="rgba(248,243,231,.8)" />
      <button type="button" className="relative px-3 py-2">PL</button>
      <span className="relative h-5 w-px bg-neutral-900/35" />
      <button type="button" className="relative px-3 py-2 text-neutral-700">EN</button>
    </div>
  );
}
...
function Badge({ text, variant = "public" }: { text: string; variant?: "public" | "unlocked" }) {
  ...
  return (
    <span className="relative inline-flex min-h-[21px] items-center gap-1 px-2 text-[9px] font-black uppercase tracking-widest text-white">
      <Frame radius={7} seed={unlocked ? 73 : 71} stroke={unlocked ? "#1748b8" : INK} strokeWidth={1} fill={unlocked ? BLUE : INK} shade />
      {unlocked && <I name="lock" className="relative h-3 w-3" stroke="white" />}
...
function Counter({ icon, value }: { icon: Icon; value: number }) {
  return (
    <span className="relative inline-flex h-9 items-center gap-2 px-3 text-sm font-black">
      <I name={icon} className="relative h-5 w-5" />
      <span className="relative min-w-3">{value}</span>
    </span>
  );
}

function VideoCard({ video, active = false, basePath }: { video: PublicVideoDTO; active?: boolean; basePath: string }) {
  const isPublic = video.tier === "PUBLIC";
  const target = video.slug || video.id;
  return (
    <Link href={`${basePath}?v=${encodeURIComponent(target)}`} className="group block">
      <article className="relative flex gap-3 p-2 transition-transform group-hover:-translate-y-0.5">
        <N4Highlight color={active ? "#cfcfcf" : "#eeeeee"} opacity={active ? 0.82 : 0.58} seed={video.id.charCodeAt(0) + 90} />
        <Frame radius={13} seed={video.id.charCodeAt(0) + 10} stroke={active ? INK : "#6b7280"} strokeWidth={active ? 1.45 : 1.05} />
        <div className="relative w-28 shrink-0 overflow-hidden rounded-[10px]">
          <div className="relative aspect-video w-full">
            {video.thumbnailUrl ? <Image src={video.thumbnailUrl} alt={video.title} fill className="object-cover" sizes="112px" /> : <div className="absolute inset-0 flex items-center justify-center bg-neutral-200 text-[9px] text-neutral-400">Video</div>}
            <div className="absolute bottom-1 left-1 z-10"><Badge text={isPublic ? "PUBLICZNE" : "ODBLOKOWANE"} variant={isPublic ? "public" : "unlocked"} /></div>
          </div>
        </div>
        <div className="relative min-w-0 flex-1 py-1 pr-1">
          <h3 className="line-clamp-2 text-[15px] font-black leading-tight tracking-wide text-neutral-950">{video.title}</h3>
          <p className="mt-2 text-xs font-bold leading-tight text-neutral-700">{video.creator?.name || "Paweł Polutek"}</p>
          <p className="mt-1 text-xs font-bold leading-tight text-neutral-600">{video.views ?? 0} wyświetleń</p>
        </div>
      </article>
    </Link>
  );
...
export default function RoughHome({ mainVideo, allVideos, currentVideoId, basePath = "/eksperyment1", experimentLabel = "eksperyment1 — layout głównej + N4/B5" }: RoughHomeProps) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (ms <= 0) return setCountdown("Premiera dostępna");
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden text-neutral-950 selection:bg-yellow-200/80" style={{ backgroundColor: PAPER, fontFamily: HAND_FONT }}>
      <div className="fixed inset-0 z-0 pointer-events-none" style={{ backgroundImage: PAPER_NOISE, opacity: 0.06 }} />
      <div className="fixed inset-0 z-0 pointer-events-none opacity-[.025]" style={{ backgroundImage: "radial-gradient(#111 .65px, transparent .65px)", backgroundSize: "22px 22px" }} />
      <div className="fixed inset-[3px] z-0 pointer-events-none"><Frame radius={7} seed={700} stroke={INK} strokeWidth={1.05} /></div>
      ...
      <nav className="sticky top-0 z-40 bg-[#f8f3e7]/92 px-4 py-3 backdrop-blur-md md:px-6">
        <div className="mx-auto flex h-[46px] max-w-6xl items-center gap-4">
          ...
          <div className="relative mx-auto hidden h-[42px] max-w-lg flex-1 items-center md:flex">
            ...
            <input readOnly placeholder="Szukaj" className="relative h-full w-full bg-transparent px-5 pr-[62px] text-base font-bold outline-none placeholder:text-neutral-700/75" />
            <span className="absolute right-[49px] top-2 h-[26px] w-px bg-neutral-900/35" />
            <I name="search" className="absolute right-4 top-1/2 h-6 w-6 -translate-y-1/2" />
          </div>
          <div className="ml-auto flex items-center gap-2"><Lang /><span className="hidden h-7 w-px bg-neutral-900/45 sm:block" /><Btn href="/" icon="login">WEJŚCIE</Btn></div>
        </div>
        <div className="mx-auto mt-2 max-w-6xl"><Separator /></div>
      </nav>

      <main className="relative z-10 mx-auto max-w-6xl px-4 pb-8 pt-6 md:px-6">
        <div className="grid grid-cols-12 gap-6">
          <section className="col-span-12 lg:col-span-8">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto] md:items-start">
              <div>
                <h1 className="text-3xl font-black leading-none tracking-[-.035em] text-neutral-950 md:text-[34px]"><Heading>{selectedVideo.title}</Heading></h1>
                <div className="mt-3 flex items-center gap-3">
                  <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-neutral-200"><Frame radius={20} seed={121} stroke={INK} strokeWidth={1.2} />{selectedVideo.creator?.imageUrl && <Image src={selectedVideo.creator.imageUrl} alt="avatar" fill className="object-cover" />}</div>
                  <div><p className="text-lg font-black leading-none">{selectedVideo.creator?.name || "Paweł Polutek"}</p><p className="mt-1 text-sm font-bold leading-none text-neutral-700">{selectedVideo.creator?.subscribersCount || 0} subskrybentów</p></div>
                  <Btn filled icon="bell" className="ml-1">Subskrajb</Btn>
                </div>
              </div>
            ...
            <div className="mt-4"><Separator label="OPIS" /></div>
            {selectedVideo.description && <div className="relative mt-3 p-4 text-base font-bold leading-relaxed text-neutral-800"><Frame radius={16} seed={177} stroke="#555" strokeWidth={1} fill="rgba(255,255,255,.2)" /><p className="relative line-clamp-3">{selectedVideo.description}</p></div>}
          </section>

          <aside className="hidden lg:col-span-4 lg:block">
            <div className="space-y-4">
              <section><h2 className="mb-1 text-lg font-black uppercase tracking-[.08em]">Publiczne</h2><Separator /><div className="mt-3 space-y-3">{publicVideos.map((v) => <VideoCard key={v.id} video={v} active={v.id === selectedVideo.id} basePath={basePath} />)}</div></section>
              {lockedVideos.length > 0 && <section><h2 className="mb-1 text-lg font-black uppercase tracking-[.08em]">Dla zalogowanych</h2><Separator /><div className="mt-3 space-y-3">{lockedVideos.map((v) => <VideoCard key={v.id} video={v} active={v.id === selectedVideo.id} basePath={basePath} />)}</div></section>}
              <section className="relative p-5"><Frame radius={16} seed={211} stroke={INK} strokeWidth={1.15} fill="rgba(248,243,231,.72)" /><div className="relative"><h2 className="text-lg font-black leading-tight tracking-wide">Wspieraj rozwój POLUTEK.PL</h2><p className="mt-3 text-sm font-bold leading-relaxed text-neutral-800">Jednorazowe wsparcie odblokowuje wszystkie materiały bonusowe — na zawsze. Bez subskrypcji.</p><div className="relative mt-3 p-3 text-center"><Frame radius={11} seed={229} stroke={INK} strokeWidth={1} fill="rgba(255,255,255,.28)" /><p className="relative text-[10px] font-black uppercase tracking-[.24em]">Do premiery</p><p className="relative mt-1 text-3xl font-black leading-none text-blue-600">{parts[0]} <span className="text-sm text-neutral-900">dni</span><span className="ml-2 text-lg text-neutral-900">{parts.slice(2).join(" ")}</span></p></div><div className="mt-3"><SupportButton /></div></div></section>
            </div>
          </aside>
        </div>
      </main>

      <div className="fixed bottom-4 right-4 z-50 hidden sm:block"><div className="relative px-3 py-2 text-[10px] font-black uppercase tracking-[.14em] text-neutral-600"><Frame radius={10} seed={301} stroke="#a3a3a3" strokeWidth={1} fill="rgba(248,243,231,.88)" /><span className="relative">{experimentLabel}</span></div></div>
    </div>
  );
}
