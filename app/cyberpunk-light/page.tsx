...
export const metadata: Metadata = {
  title: `Cyberpunk (jasny) — ${APP_NAME}`,
  description: 'Eksperymentalna odsłona strony głównej — ta sama kolorystyka i jasne tło, stylistyka cyberpunk.',
  robots: { index: false, follow: false },
};

/**
 * Design experiment: the exact same homepage data/logic/components as `/`
 * (Navbar, ChannelHome, Footer), wrapped in the `.cyberpunk-light-page` CSS
 * scope from globals.css. Keeps the existing accent blue and light
 * background; the cyberpunk read comes from monospace/bracketed nav
 * labels, a hairline grid texture, glowing HUD corner brackets, and a
 * scan-line sweep — not a dark/neon palette swap. No component logic is
 * duplicated here — only the visual skin differs. See also `/cyberpunk`,
 * the dark neon (cyan/magenta) variant.
 */
export default async function CyberpunkLightPage(props: { searchParams: Promise<{ v?: string, q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (e) {
      logger.error("[CYBERPUNK_LIGHT_AUTH_ERROR]", e);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((e) => {
      logger.error("[CYBERPUNK_LIGHT_CURRENT_USER_ERROR]", e);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => {
      logger.error("[CYBERPUNK_LIGHT_USER_FETCH_ERROR]", e);
    });
  ...
  if (content.status === 'error' || content.status === 'empty') {
    ...
    return (
      <div className="cyberpunk-light-page">
        <Navbar />
