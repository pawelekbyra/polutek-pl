...
export const metadata: Metadata = {
  title: `Cyberpunk — ${APP_NAME}`,
  description: 'Eksperymentalna, ciemna neonowa odsłona strony głównej.',
  robots: { index: false, follow: false },
};

/**
 * Design experiment: the exact same homepage data/logic/components as `/`
 * (Navbar, ChannelHome, Footer), wrapped in the `.cyberpunk-dark-page` CSS
 * scope from app/cyberpunk.css — a dark neon (cyan/magenta) re-skin. No
 * component logic is duplicated here — only the visual skin differs.
 * See also `/cyberpunk-light`, the same experiment keeping the site's
 * normal light background and blue accent.
 */
export default async function CyberpunkPage(props: { searchParams: Promise<{ v?: string, q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (e) {
      logger.error("[CYBERPUNK_AUTH_ERROR]", e);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((e) => {
      logger.error("[CYBERPUNK_CURRENT_USER_ERROR]", e);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => {
      logger.error("[CYBERPUNK_USER_FETCH_ERROR]", e);
    });
  ...
  if (content.status === 'error' || content.status === 'empty') {
    ...
    return (
      <div className="cyberpunk-dark-page">
        <Navbar />
