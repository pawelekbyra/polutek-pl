...
import ExperimentSevenSkin from "./ExperimentSevenSkin";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Eksperyment 7 — bauhaus blocks · Polutek",
  description: "Strona główna Polutek w geometrycznej, blokowej stylizacji inspirowanej plakatem.",
};

export default async function Eksperyment7Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (error) {
      logger.error("[EXPERIMENT7_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT7_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT7_USER_FETCH_ERROR]", error);
    });
