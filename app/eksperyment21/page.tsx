...
export const metadata: Metadata = {
  title: "Eksperyment 21 — spokojny szkic · Polutek",
...
export default async function Eksperyment21Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  ...
  const getSafeAuth = async () => {
    try {
      ...
      logger.error("[EXPERIMENT21_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT21_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT21_USER_FETCH_ERROR]", error);
    });
