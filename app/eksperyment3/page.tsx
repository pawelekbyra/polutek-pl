...
import ExperimentThreeSkin from "./ExperimentThreeSkin";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Eksperyment 3 — katalog cards · Polutek",
  description: "Strona Polutek jako zestaw kart, etykiet i warstw markerowych z /katalog.",
};

export default async function Eksperyment3Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (error) {
      logger.error("[EXPERIMENT3_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT3_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT3_USER_FETCH_ERROR]", error);
    });
