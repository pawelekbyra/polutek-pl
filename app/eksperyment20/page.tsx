...
export const metadata: Metadata = {
  title: "Eksperyment 20 — bauhaus bez czerwieni · Polutek",
...
export default async function Eksperyment20Page(props: { searchParams: Promise<{ v?: string; q?: string }> }) {
  ...
  const getSafeAuth = async () => {
    try {
      ...
      logger.error("[EXPERIMENT20_AUTH_ERROR]", error);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((error) => {
      logger.error("[EXPERIMENT20_CURRENT_USER_ERROR]", error);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((error) => {
      logger.error("[EXPERIMENT20_USER_FETCH_ERROR]", error);
    });
