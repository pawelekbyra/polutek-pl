...
export function ChannelSettingsForm({
  ...
  const [displaySubscribersCount, setDisplaySubscribersCount] = useState(
    initialCreator?.displaySubscribersCount?.toString() || "",
  );
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/channel", {
        ...
        body: JSON.stringify({
          ...
          displaySubscribersCount:
            displaySubscribersCount.trim() === ""
              ? null
              : Number(displaySubscribersCount),
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error || "Nie udało się zapisać danych kanału.",
        );
      }
      ...
      setDisplaySubscribersCount(
        payload.creator.displaySubscribersCount?.toString() || "",
      );
      setStatus("saved");
    } catch (saveError) {
      setError(
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <AdminNavigation
          items={[
            ...
            { label: "Kanał", href: "/admin/channel" },
          ]}
          backHref="/admin"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Nie udało się przygotować głównego kanału. Sprawdź konfigurację
              MAIN_CREATOR_SLUG albo spróbuj ponownie po odświeżeniu panelu.
            </CardDescription>
          </CardHeader>
        </Card>
      </main>
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 rounded-3xl border bg-card p-6 shadow-sm md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Nazwa kanału, opis, cover photo i podstawowe ustawienia profilu
          publicznego.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Settings className="h-5 w-5" /> Dane kanału
            </CardTitle>
            <CardDescription>
              Slug jest stały dla monokanału i pochodzi z konfiguracji, dlatego
              nie jest edytowany w tym formularzu.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="real-subscribers">
                      Liczba subskrybentów (licznik cache)
                    </Label>
                    <button
                      type="button"
                      onClick={async () => {
                  ...
                  <Input
                    id="real-subscribers"
                    value={creator.subscribersCount}
                    disabled
                    className="bg-muted"
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="display-subscribers">
                    Wyświetlana liczba subskrybentów (Fake)
                  </Label>
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(event) =>
                        setDisplaySubscribersCount(event.target.value)
                      }
                  ...
                  <p className="text-[10px] text-muted-foreground leading-tight">
                    Jeśli wpiszesz liczbę, zostanie ona wyświetlona publicznie
                    zamiast licznika cache.
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="text-sm">
                  ...
                  {status === "idle" && (
                    <span className="text-muted-foreground">
                      Zmieniasz tylko publiczne, dozwolone pola kanału.
                    </span>
                  )}
                  {status === "saving" && (
                    <span className="text-muted-foreground">
        ...
        <aside className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ImageIcon className="h-5 w-5" /> Avatar
              </CardTitle>
              <CardDescription>
                Model kanału nie ma osobnego pola avatarUrl, więc publiczny
                avatar korzysta z bezpiecznego fallbacku użytkownika.
              </CardDescription>
            </CardHeader>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-xs leading-5 text-muted-foreground">
                    Jeśli nie ustawisz avatara kanału, zostanie użyte zdjęcie
                    profilowe z konta właściciela kanału w Clerk.
                  </p>
                </div>
              </div>
            </CardContent>
