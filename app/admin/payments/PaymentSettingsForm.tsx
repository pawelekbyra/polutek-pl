...
export function PaymentSettingsForm({ initialLimits }: Props) {
  const [limits, setLimits] =
    useState<Record<SupportedCurrency, CurrencyLimit>>(initialLimits);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          payload?.error || "Nie udało się zapisać ustawień płatności.",
        );
      ...
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Nie udało się zapisać ustawień płatności.",
      );
      setStatus("error");
    }
  }
    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 rounded-3xl border bg-card p-6 shadow-sm md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Edytuj minimalną kwotę napiwku osobno dla każdej waluty. Udana
          płatność automatycznie nadaje status Patrona.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Settings className="h-5 w-5" /> Minimalne kwoty
          </CardTitle>
          <CardDescription>
            Kwoty są zapisywane dynamicznie i używane zarówno w interfejsie, jak
            i w walidacji API checkoutu.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    onChange={(event) => {
                      ...
                      setLimits((current) => ({
                        ...
                        [currency]: {
                          ...
                          minAmountMinor: Number.isFinite(next)
                            ? Math.round(next * 100)
                            : 0,
                        },
                      }));
            ...
            <div className="flex flex-col gap-3 rounded-2xl border bg-muted/40 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-sm">
                {status === "saved" && (
                  <span className="text-green-600">Zapisano ustawienia.</span>
                )}
                {status === "error" && (
                ...
                {status === "idle" && (
                  <span className="text-muted-foreground">
                    CHF i GBP są dostępne w bramce obok PLN, EUR i USD.
                  </span>
                )}
                {status === "saving" && (
