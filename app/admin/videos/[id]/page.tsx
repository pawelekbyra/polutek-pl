...
import {
  ...
  type VideoDetailsTab,
} from "./details-tab-state";
...
export default function VideoDetailsPage(props: {
  ...
  const handleAction = async (action: string, extraBody: any = {}) => {
    try {
      ...
      const url = isFullUrl
        ? `/api/admin/${action}`
        : `/api/admin/videos/${params.id}/actions`;
        ...
        <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 text-center py-20">
          ...
          <AdminNavigation
            items={[
              ...
              { label: "Szczegóły", href: `/admin/videos/${params.id}` },
            ]}
          ...
          <div className="flex flex-col gap-1">
            <AdminNavigation
              items={[
                { label: "Admin", href: "/admin" },
                { label: "Filmy", href: "/admin/videos" },
                { label: "Szczegóły", href: `/admin/videos/${video.id}` },
              ]}
              backHref="/admin/videos"
              backLabel="Wróć do filmów"
              className=""
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="default" size="sm" asChild>
              <Link href={`/admin/videos/${video.id}/edit`}>
                <Edit className="mr-2 h-4 w-4" /> Edytuj dane
              </Link>
            </Button>
          ...
          <div className="lg:w-3/4 space-y-6">
            ...
            >
              <TabsList className="w-full justify-start bg-background border rounded-lg p-1 mb-6 h-auto flex-wrap">
                ...
                >
                  <Layout className="h-4 w-4" /> Podsumowanie
                </TabsTrigger>
                <TabsTrigger
                  value="content"
                  className="px-4 py-2 flex gap-2 items-center"
                >
                  <FileVideo className="h-4 w-4" /> Treść
                </TabsTrigger>
                <TabsTrigger
                  value="media"
                  className="px-4 py-2 flex gap-2 items-center"
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2 space-y-6">
                    ...
                    {video.status === "DRAFT" &&
                      (!video.asset ||
                        video.asset.processingState !== "READY") && (
                        <VideoUploadSection
                          videoId={video.id}
                  ...
                  <Card className="shadow-sm">
                    ...
                    <CardContent className="space-y-4">
                      <div className="flex justify-between py-2 border-b">
                        ...
                        <Badge
                          variant={
                            video.status === "PUBLISHED" ? "default" : "outline"
                          }
                        >
                          {video.status}
                        </Badge>
                      ...
                      <div className="flex justify-between py-2 border-b">
                        ...
                        {video.isMainFeatured ? (
                          <Badge className="bg-blue-600">TAK</Badge>
                        ) : (
                          <span className="text-xs opacity-50">Nie</span>
                        )}
                      </div>
              ...
              <TabsContent value="media" id="media" className="space-y-6">
                {video.status === "DRAFT" &&
                  (!video.asset || video.asset.processingState !== "READY") && (
                    <VideoUploadSection
                      videoId={video.id}
                      onUploadComplete={fetchVideo}
                      initialAsset={video.asset}
                    ...
                    <Card className="shadow-sm">
                      <CardHeader className="flex flex-row items-center justify-between">
                        <CardTitle className="text-lg">
                          Zasób Cloudflare Stream
                        </CardTitle>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleAction("sync-cloudflare")}
                            disabled={
                              video.asset?.provider !== "CLOUDFLARE_STREAM"
                            }
                          >
                            <RotateCcw className="mr-2 h-3 w-3" /> Synchronizuj
                            status
                          </Button>
                        </div>
                      </CardHeader>
                        ...
                        {video.asset?.provider === "CLOUDFLARE_STREAM" ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                              ...
                              <div className="space-y-1">
                                ...
                                <div>
                                  {video.asset.isPrimary ? (
                                    <Badge className="bg-green-600">
                                      TAK — używany do odtwarzania
                                    </Badge>
                                  ) : (
                                    <Badge variant="outline">
                                      NIE — nie używany do odtwarzania
                                    </Badge>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="space-y-4">
                              <div className="space-y-1">
                                ...
                                <div className="text-sm">
                                  {formatDate(video.asset.providerSyncedAt)}
                                </div>
                              </div>
                              <div className="space-y-1">
                                <Label className="text-[10px] uppercase font-bold text-muted-foreground">
                                ...
                                <div className="text-sm">
                                  {formatDate(video.asset.processingStartedAt)}
                                </div>
                              </div>
                              <div className="space-y-1">
                                <Label className="text-[10px] uppercase font-bold text-muted-foreground">
                                ...
                                <div className="text-sm">
                                  {formatDate(video.asset.processingEndedAt) ||
                                    "W toku..."}
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="py-12 text-center border-dashed border-2 rounded-xl bg-muted/20">
                            ...
                            <p className="text-sm text-muted-foreground italic">
                              Brak przypisanego zasobu Cloudflare Stream.
                            </p>
                          </div>
                        )}
                          ...
                          <div className="flex flex-wrap gap-3">
                            {video.videoUrl && !video.asset && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleAction("import-legacy-to-cloudflare", {
                                    publishAfterAssetReady: true,
                                  })
                                }
                              >
                                <Send className="mr-2 h-4 w-4" /> Importuj
                                legacy URL do Cloudflare
                              </Button>
                            )}
                            <Button
                              variant="outline"
                              ...
                              onClick={() => {
                                const uid = window.prompt(
                                  "Podaj Cloudflare Stream UID:",
                                );
                                if (uid)
                                  handleAction("attach-asset", {
                                    providerAssetId: uid,
                                    publishAfterAssetReady: true,
                                  });
                              }}
                            >
                              <Settings className="mr-2 h-4 w-4" /> Podepnij
                              ręcznie UID
                            </Button>
                          </div>
                        </div>
                    ...
                    <Card className="shadow-sm">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {video.publishAfterAssetReady && (
                          <div className="space-y-3">
                            <div className="text-[10px] text-muted-foreground space-y-1">
                              <p>
                                Status:{" "}
                                {video.publishAfterAssetReadyCompletedAt
                                  ? "ZAKOŃCZONO"
                        ...
                        <div className="rounded-lg bg-sky-50 p-3 text-[10px] text-sky-950">
                          ...
                          <p>
                            Gdy zasób Cloudflare Stream osiągnie stan{" "}
                            <strong>READY</strong>, backend automatycznie
                            opublikuje film, jeśli ta opcja jest aktywna.
                          </p>
                        </div>
                      </CardContent>
                    </Card>
              ...
              <TabsContent value="access" className="space-y-6">
                <Card className="shadow-sm">
                  <CardHeader>
                    <CardTitle className="text-lg">
                      Zarządzanie dostępem i publikacją
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4 border rounded-xl p-4">
                        ...
                        <p className="text-[10px] text-muted-foreground leading-relaxed mt-4">
                          Zmiana poziomu dostępu wpływa na to, kto może obejrzeć
                          ten materiał. Poziom dostępu edytujesz w kanonicznym
                          formularzu filmu.
                        </p>
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/admin/videos/${video.id}/edit`}>
                            <Edit className="mr-2 h-3 w-3" /> Przejdź do edycji
                            dostępu
                          </Link>
                        </Button>
                      </div>
                    ...
                    {!video.diagnostics || video.diagnostics.length === 0 ? (
                      <div className="flex items-center gap-4 p-6 bg-green-50 text-green-700 rounded-xl border border-green-100">
                        ...
                        <div>
                          ...
                          <p className="text-xs opacity-80 mt-0.5">
                            Wszystkie pola i metadane wyglądają na poprawne.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {video.diagnostics.map((d: any, idx: number) => (
                          <div
                            key={idx}
                            className={`flex items-start gap-4 p-5 rounded-xl border-l-4 ${d.severity === "ERROR" ? "bg-red-50 border-red-500 text-red-800" : "bg-amber-50 border-amber-400 text-amber-800"}`}
                          >
                            <AlertTriangle
                              className={`h-6 w-6 shrink-0 ${d.severity === "ERROR" ? "text-red-500" : "text-amber-500"}`}
                            />
                            <div className="flex-1">
                              ...
                              {d.field && (
                                <p className="text-[10px] opacity-70 mt-2 font-mono bg-white/50 w-fit px-1 rounded uppercase tracking-tighter">
                                  Pole: {d.field}
                                </p>
                              )}
                            </div>
                ...
                <Card className="shadow-sm">
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Ostatnie dyskusje pod tym filmem.
                      </CardDescription>
                    </div>
                    <Badge variant="outline">
                      {video._count?.comments || 0} komentarzy
                    </Badge>
                  </CardHeader>
                  <CardContent>
                    {video.comments?.length > 0 ? (
                      <div className="space-y-4">
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              {comment.status === "VISIBLE" ? (
                                <Button
                                  onClick={() =>
                                    handleAction(`comments/${comment.id}/hide`)
                                  }
                                  variant="ghost"
                                  size="sm"
                              ...
                              <Button
                                onClick={() =>
                                  handleAction(`comments/${comment.id}/delete`)
                                }
                                variant="ghost"
                                size="sm"
                              ...
                              >
                                <Link
                                  href={`/watch/${video.slug}#comment-${comment.id}`}
                                  target="_blank"
                                >
                                  Pokaż w serwisie
                                </Link>
                        ...
                        >
                          <Link
                            href={`/admin/comments?videoId=${encodeURIComponent(video.id)}`}
                          >
                            Pokaż wszystkie w moderacji
                          </Link>
                        </Button>
                      </div>
                    ) : (
                      <div className="py-12 text-center space-y-2 opacity-50">
                        ...
                        <p className="text-sm italic">
                          Brak komentarzy pod tym filmem.
                        </p>
                      </div>
                    )}
                  </CardContent>
              ...
              <TabsContent value="stats" className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <StatBlock
                    label="Sesje wideo"
                    value={video._count?.playbackSessions || 0}
                  ...
                  <StatBlock
                    label="Komentarze"
                    value={video._count?.comments || 0}
                    icon={<MessageSquare className="h-4 w-4" />}
                  />
                </div>
                <p className="text-xs text-muted-foreground italic">
                  Rozszerzona analityka sesji będzie dodana w osobnym zakresie.
                </p>
              </TabsContent>

              <TabsContent value="audit">
                <VideoAuditLog logs={video.auditLogs} />
              </TabsContent>
            </Tabs>
          </div>
          ...
          <div className="lg:w-1/4 space-y-6">
            <Card className="shadow-sm border-0 bg-primary text-primary-foreground overflow-hidden relative">
              ...
              <CardContent className="space-y-4 relative">
                <Button
                  variant="secondary"
                  className="w-full justify-start h-10 font-bold"
                  onClick={() => handleAction("set-hero")}
                  disabled={video.isMainFeatured}
                >
                ...
                >
                  <Link href={`/admin/videos/${video.id}/edit`}>
                    <Edit className="mr-2 h-4 w-4" /> Edytuj dane filmu
                  </Link>
                </Button>
              </CardContent>
            </Card>
            <Card className="shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant={video.showInSidebar ? "default" : "outline"}>
                    {video.showInSidebar ? "TAK" : "NIE"}
                  </Badge>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href="/admin/videos/layout">
                    Zmień kolejność hurtowo
                  </Link>
                </Button>
              </CardContent>
            </Card>
