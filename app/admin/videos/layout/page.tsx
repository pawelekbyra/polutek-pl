...
export default function ChannelLayoutPage() {
  ...
  const fetchVideos = async () => {
    ...
    try {
      const res = await fetch(
        "/api/admin/videos?pageSize=100&orderBy=sidebarOrder&orderDir=asc",
      );
      if (res.ok) {
  ...
  const toggleSidebar = (video: AdminVideoListItem) => {
    setVideos((prev) =>
      prev.map((v) =>
        v.id === video.id ? { ...v, showInSidebar: !v.showInSidebar } : v,
      ),
    );
  };
      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-6">
          <AdminNavigation
            items={[
              ...
              { label: "Układ kanału", href: "/admin/videos/layout" },
            ]}
        ...
        <div className="mb-8 rounded-3xl border bg-card p-6 shadow-sm md:p-8 text-center md:text-left">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            Zmieniaj kolejność filmów strzałkami, wybierz główny film Hero
            (tylko publiczne) i zarządzaj widocznością w sidebarze.
          </p>
        </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 ml-6">
                      ...
                      {video.isMainFeatured && (
                        <Badge className="bg-blue-600 text-[8px]">
                          <Star className="h-2 w-2 mr-1 fill-current" /> HERO
                        </Badge>
                      )}
                    </div>
                  </div>
