...
export function ActivePlaybackRouteCard({ route }: { route: VideoMediaState["activeRoute"] }) {
  return <div className="rounded-lg border bg-card p-3">
    ...
    <p className="mt-2 text-xs text-muted-foreground">{route ? `Wybrane przez: ${route.activatedBy}` : "Źródło odtwarzania zostanie wybrane po przygotowaniu dostawcy albo ręcznie w trybie zaawansowanym."}</p>
  </div>;
}
