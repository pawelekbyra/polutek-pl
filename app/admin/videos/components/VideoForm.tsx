        ...
        <div className="space-y-8">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2"><Label>{isCreate ? "Docelowy stan po zapisie" : "Status"}</Label><Select value={formData.status ?? ""} onValueChange={v => setFormData({...formData, status: v ?? ""})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="DRAFT">Szkic</SelectItem><SelectItem value="PUBLISHED">Opublikowany</SelectItem><SelectItem value="ARCHIVED">Zarchiwizowany</SelectItem></SelectContent></Select><p className="text-xs text-muted-foreground">Publikacja pozostaje osobną akcją w szczegółach filmu.</p></div>
