...
} from "@/lib/services/admin/payments-admin.dto";
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <AdminNavigation
          items={[
            ...
            { label: "Wpłaty", href: "/admin/users/payments" },
          ]}
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground leading-relaxed">
            Monitoruj transakcje Stripe, weryfikuj statusy płatności i sumy
            przychodów.
          </p>
        </div>
              ...
              <Card key={f.currency} className="border-0 shadow-sm">
                ...
                <CardContent>
                  <div className="text-2xl font-black">
                    {(f.amountMinor / 100).toLocaleString("pl-PL", {
                      minimumFractionDigits: 2,
                    })}{" "}
                    {f.currency}
                  </div>
                  <div className="text-[10px] text-muted-foreground mt-1 font-medium italic">
                    Filtrowane podsumowanie
                  </div>
                </CardContent>
              </Card>
            ))}
            {summary.totalRefunded.some((r: any) => r.amountMinor > 0) &&
              summary.totalRefunded.map(
                (r: any) =>
                  r.amountMinor > 0 && (
                    <Card
                      key={`ref-${r.currency}`}
                      className="border-0 shadow-sm border-l-4 border-red-500"
                    >
                      <CardHeader className="pb-2">
                        <CardTitle className="text-[10px] font-bold text-red-600 uppercase tracking-widest">
                          Zwroty {r.currency}
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="text-2xl font-black text-red-600">
                          {(r.amountMinor / 100).toLocaleString("pl-PL", {
                            minimumFractionDigits: 2,
                          })}{" "}
                          {r.currency}
                        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start">
            ...
            <div className="flex flex-wrap gap-2 items-center flex-1">
              ...
              <Select
                ...
                onValueChange={(v) => setCurrencyFilter(v || "ALL")}
              >
              ...
              <div className="flex items-center space-x-2 bg-background/50 px-3 py-1.5 rounded-md border h-9">
                <Checkbox
                  ...
                  onCheckedChange={(v) => setRefundedOnly(!!v)}
                />
                <Label
                  htmlFor="refunded"
                    ...
                    payments.map((p) => (
                      <TableRow key={p.id}>
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="h-7 w-7 rounded-full overflow-hidden bg-muted relative border shrink-0">
                              <div className="flex items-center justify-center h-full text-[10px] font-bold uppercase">
                                {p.email[0]}
                              </div>
                            </div>
                            <div className="flex flex-col min-w-0">
                              <Link
                                ...
                                className="font-bold text-xs hover:underline decoration-primary truncate"
                              >
                                {p.email}
                              </Link>
                        ...
                        <TableCell>
                          {p.creator ? (
                            <Link
                              href={`/channel/${p.creator.slug}`}
                              className="inline-flex items-center gap-1.5 hover:underline"
                            >
                              <Badge
                                variant="outline"
                                className="text-[9px] font-bold uppercase py-0 px-1.5 border-amber-200 text-amber-700 bg-amber-50"
                              >
                                {p.creator.name}
                              </Badge>
                            </Link>
                          ...
                          <div className="flex flex-col">
                            <div
                              className={
                                p.status === "SUCCEEDED"
                                  ? "font-black text-sm"
                                  : "font-medium text-sm opacity-60"
                              }
                            >
                              {(p.amountMinor / 100).toLocaleString("pl-PL", {
                                minimumFractionDigits: 2,
                              })}{" "}
                              {p.currency}
                            </div>
                            {p.refundedAmountMinor > 0 && (
                              <div className="text-[9px] text-red-600 font-bold uppercase mt-0.5">
                                Zwrócono:{" "}
                                {(p.refundedAmountMinor / 100).toFixed(2)}
                              </div>
                            )}
                          </div>
                        </TableCell>
