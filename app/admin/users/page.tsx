...
export default function AdminUsersPage() {
  ...
  const [hasSubscriptionsFilter, setHasSubscriptionsFilter] =
  ...
  const fetchUsers = useCallback(
    async (p = page) => {
      ...
      try {
        ...
        if (patronFilter !== "ALL")
          url += `&isPatron=${patronFilter === "PATRON"}`;
        if (languageFilter !== "ALL") url += `&language=${languageFilter}`;
        if (patronSourceFilter !== "ALL")
          url += `&patronSource=${patronSourceFilter}`;
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">
            Użytkownicy i Patroni
          </h1>
          <p className="mt-1 text-sm text-muted-foreground leading-relaxed">
            Zarządzaj społecznością, weryfikuj statusy patronów i monitoruj
            aktywność finansową.
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex flex-col lg:flex-row gap-4 items-start">
            <form
              ...
              className="w-full lg:w-1/3 flex gap-2"
            >
            ...
            <div className="flex flex-wrap gap-2 items-center flex-1">
              ...
              <Select
                value={patronFilter}
                onValueChange={(v) => setPatronFilter(v || "ALL")}
              >
                <SelectTrigger className="w-[120px] h-9 text-xs">
              ...
              <Select
                value={patronSourceFilter}
                onValueChange={(v) => {
                  setPatronSourceFilter(v || "ALL");
                  setPage(1);
                }}
              >
                <SelectTrigger className="w-[130px] h-9 text-xs">
                  <SelectValue placeholder="Źródło Patronatu" />
                </SelectTrigger>
                <SelectContent>
              ...
              <Select
                value={orderBy}
                onValueChange={(v) => setOrderBy(v || "createdAt")}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="patronSince">
                    Od kiedy patron (aktywny grant)
                  </SelectItem>
              ...
              <div className="flex flex-wrap gap-4 items-center bg-background/50 px-3 py-1.5 rounded-md border h-9">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setIsDeletedFilter(!!v)}
                  />
                  <Label
                    htmlFor="deleted"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="payments"
                    checked={hasPaymentsFilter}
                    onCheckedChange={(v) => setHasPaymentsFilter(!!v)}
                  />
                  <Label
                    htmlFor="payments"
                    className="text-[10px] font-bold uppercase cursor-pointer"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="subs"
                    checked={hasSubscriptionsFilter}
                    onCheckedChange={(v) => setHasSubscriptionsFilter(!!v)}
                  />
                  <Label
                    htmlFor="subs"
                    className="text-[10px] font-bold uppercase cursor-pointer"
                ...
                <TableBody>
                  {error ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={6}
                        className="py-20 text-center italic text-muted-foreground animate-pulse"
                      >
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col min-w-0">
                              <Link
                                ...
                                className="font-bold text-sm hover:underline decoration-primary truncate"
                              >
                                {user.email}
                              </Link>
                              <div className="text-[10px] text-muted-foreground truncate">
                                {user.name || user.username || "Bez nazwy"}
                              </div>
                              {user.isDeleted && (
                                <Badge
                                  variant="destructive"
                                  className="w-fit h-4 text-[8px] mt-1"
                                >
                                  USUNIĘTY
                                </Badge>
                              )}
                          ...
                          <div className="flex flex-col gap-1.5">
                            {user.patronTruth?.isPatron ? (
                              <div className="flex flex-col gap-0.5">
                                <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100 border-amber-200 text-[10px] w-fit">
                                  PATRON (GRANT)
                                </Badge>
                                <div className="text-[9px] text-muted-foreground">
                                  Aktywny grant od:{" "}
                                  {formatDate(
                                    user.activeGrantSince ||
                                      user.patronTruth.firstActiveGrantAt,
                                  )}
                                </div>
                                <div className="text-[8px] uppercase font-bold opacity-60">
                                  Źródło grantu:{" "}
                                  {user.activeGrantSource ||
                                    user.patronTruth.source ||
                                    "ACTIVE_GRANT"}{" "}
                                  · aktywne:{" "}
                                  {user.activeGrantCount ??
                                    user.patronTruth.activeGrantCount}
                                </div>
                                {user.patronCacheTruthMismatch && (
                                  <Badge
                                    variant="destructive"
                              ...
                              <div className="flex flex-col gap-0.5">
                                <Badge
                                  variant="outline"
                                  className="text-[10px] w-fit opacity-50"
                                >
                          ...
                          <div className="flex flex-col gap-1">
                              ...
                              <span className="text-[11px] font-bold">
                                {user.paymentTotals?.length > 0
                                  ? user.paymentTotals
                                      .map(
                                        (t: any) =>
                                          `${(t.totalPaidMinor / 100).toFixed(2)} ${t.currency}`,
                                      )
                                      .join(", ")
                                  : "0.00 PLN"}
                              </span>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {user.hasSubscriptions && (
                                <Badge
                                  variant="outline"
                                  className="h-4 text-[8px] bg-blue-50 text-blue-700 border-blue-100"
                                >
                                  FOLLOW
                                </Badge>
                              )}
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-2">
                            <Badge
                              ...
                              className="text-[9px] w-fit font-mono"
                            >
                              {user.role}
                            </Badge>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <UserPatronActions
                              userId={user.id}
                              isPatron={user.patronTruth?.isPatron === true}
                            />
                            <Button
                              variant="ghost"
                              size="icon"
                            ...
                            >
                              <Link href={`/admin/emails?to=${user.email}`}>
                        ...
                        <TableCell className="text-right">
                          ...
                          <div className="text-[9px] text-muted-foreground italic mt-0.5">
                            Zaktualizowano: {formatDate(user.updatedAt)}
                          </div>
                        </TableCell>
                      </TableRow>
                    ))
                  ...
                  {!isLoading && users.length === 0 && (
                    <TableRow>
                      <TableCell
                        colSpan={6}
                        className="py-20 text-center text-muted-foreground italic border-b-0"
                      >
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 py-8 border-t bg-muted/10">
                ...
                <div className="text-sm font-medium">
                  Strona <span className="text-primary font-bold">{page}</span>{" "}
                  z {totalPages}
                </div>
                <Button
                  variant="outline"
    ...
    <Card className="shadow-sm border-0">
      <CardContent className="pt-6">
        <div className="flex items-center gap-4">
          <div
            className={`p-2.5 rounded-xl border ${colors[color] || colors.blue}`}
          >
            {icon}
          </div>
          <div>
    ...
    <Card className="shadow-sm border-0">
      <CardContent className="flex h-full items-start justify-between gap-4 p-5">
        <div className="flex gap-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border bg-blue-100 text-blue-600">
            {icon}
          </div>
          <div>
        ...
        <Button variant="outline" size="sm" asChild className="shrink-0">
          <Link href={href}>
            Otwórz <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </Button>
      </CardContent>
    </Card>
