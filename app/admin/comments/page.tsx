...
export default function AdminCommentsPage() {
  ...
  const handleAction = async (
    ...
    try {
      const res = await fetch(`/api/admin/comments/${commentId}/${action}`, {
        method: "POST",
      });
      if (res.ok) {
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <AdminNavigation
          items={[
            ...
            { label: "Komentarze", href: "/admin/comments" },
          ]}
          backHref="/admin"
        ...
        <div className="flex flex-col justify-between gap-4 mb-8 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Przeglądaj, ukrywaj, przywracaj i usuwaj komentarze.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            />
            <Button onClick={refreshComments} variant="secondary">
              <Search size={16} />
            </Button>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {videoId
                ? "Zarządzaj komentarzami pod wybranym filmem."
                : "Zarządzaj dyskusjami w całej aplikacji."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
