...
export const metadata: Metadata = {
  title: `Połysk 2 — ${APP_NAME}`,
  description: 'Eksperymentalna odsłona strony głównej — te same kolory co main, dopieszczona głębszym połyskiem (glossy lacquer).',
  robots: { index: false, follow: false },
};

/**
 * Design experiment: the exact same homepage data/logic/components as `/`
 * (Navbar, ChannelHome, Footer), wrapped in the `.polysk2-page` CSS scope
 * from globals.css — "glossy lacquer" polish. Same colors/tokens as main,
 * purely additive (deeper elevation shadows, a specular highlight band
 * across cards/buttons, a beveled accent button, a soft contact-shadow
 * under the video frame). No component logic is duplicated here — only
 * the visual polish differs. See also `/polysk1`, the airier "frosted
 * glass" take.
 */
export default async function Polysk2Page(props: { searchParams: Promise<{ v?: string, q?: string }> }) {
  const searchParams = await props.searchParams;
  const getSafeAuth = async () => {
    try {
      return await auth();
    } catch (e) {
      logger.error("[POLYSK2_AUTH_ERROR]", e);
  ...
  const [authData, content, user] = await Promise.all([
    ...
    currentUser().catch((e) => {
      logger.error("[POLYSK2_CURRENT_USER_ERROR]", e);
      return null;
    }),
  ]);
  ...
  if (userId) {
    ...
    await getOrCreateCurrentUser(userCtx, userId).catch((e) => {
      logger.error("[POLYSK2_USER_FETCH_ERROR]", e);
    });
